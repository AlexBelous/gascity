...
| `session_start_reconciler` | string |  | `off` | SessionStartReconciler selects ownership of session-start causes during the keyed reconciler rollout. "off" (the default) keeps every start on the legacy fleet-wide reconciler. "auto" selects the keyed path when its boot-time requirements are available and otherwise degrades loudly. "require" refuses startup rather than falling back when those requirements are unavailable. The value is boot-latched; changing it requires a controller restart. Enum: `off`, `auto`, `require` |
