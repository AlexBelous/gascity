...
func TestCrashBeforeRunClosedFromSealSnapshot(t *testing.T) {
	ctx := context.Background()
	// All-pass exec/do chain; snapshotting at every unit boundary means the seal
	// snapshot anchors the fully-settled state the crash strands unsealed.
	doc := decodeIR(t, blockDoc("seal",
		execNode("A", `echo a`, nil),
		execNode("B", `echo b`, []string{"A"}),
		doNode("C", "Seal {{B}}.", []string{"B"}),
	))

	// Genesis reference: an uninterrupted snapshotting run to the seal.
	gStore := newStore(t)
	gRes, err := engine.RunWithOptions(ctx, gStore, doc, nil, engine.Options{
		Host:          passDoStub("C"),
		SnapshotEvery: 1,
	})
	if err != nil {
	...
	host := passDoStub("C")
	resumed, store, stream := injectCrashThenResume(
		t,
		doc,
		host,
		engine.CrashBeforeRunClosed,
		"",
		1,
	)
	if calls := host.Calls(); len(calls) != 1 {
		t.Fatalf("C host calls across crash+resume = %d, want exactly 1", len(calls))
	}
	...
	snapshots := allSnapshots(t, store, stream)
	if len(snapshots) == 0 {
		t.Fatalf("no snapshot anchored before the crash — the resume-from-seal path was not exercised")
	}
	assertSnapshotsWellFormed(t, store, stream)
	var sealedState struct {
		Nodes map[string]struct {
			Result *engine.LumenStepResult `json:"result"`
		} `json:"nodes"`
	}
	if err := json.Unmarshal(snapshots[len(snapshots)-1].state, &sealedState); err != nil {
		t.Fatalf("decode seal snapshot: %v", err)
	}
	if result := sealedState.Nodes["C:0"].Result; result == nil {
		t.Fatal(`seal snapshot nodes["C:0"].result is absent`)
	} else {
		requireResultJSON(t, *result,
			`{"value":"summary:C","outcome":"succeeded","error":null}`)
	}
	...
	genesisResult, err := json.Marshal(gRes.Result)
	if err != nil {
		t.Fatalf("marshal genesis result: %v", err)
	}
	resumedResult, err := json.Marshal(resumed.Result)
	if err != nil {
		t.Fatalf("marshal resumed result: %v", err)
	}
	if string(resumedResult) != string(genesisResult) {
		t.Errorf("run result = %s, want uninterrupted genesis %s", resumedResult, genesisResult)
	}
	if resumed.Outcome != engine.OutcomePass {
		t.Errorf("run outcome = %q, want pass", resumed.Outcome)
	}
