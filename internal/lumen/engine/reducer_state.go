...
// lumenState is the reducer v7 carried-forward state (blueprint §2.1): the run
...
// all drain members settled. A non-scatter aggregate additionally requires at
// least one member to have run. Structural scatter is ready even when every
// member skipped because it still produces a successful result harvest.
func (s *lumenState) ready(n *nodeState) bool {
	...
	if len(n.Members) > 0 {
		...
		if !anyRan && n.Kind != string(ir.NodeScatter) {
			return false
		}
	}
	return true
