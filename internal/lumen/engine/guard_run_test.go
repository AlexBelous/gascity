...
// the then. The scatter's bare value is its structured harvest, not an empty
// aggregate sentinel.
func TestGuardInSubFormulaAggregateOutcomeRef(t *testing.T) {
	...
	// Two guards over the sub-scatter: draft reads its .outcome, while bare proves
	// the harvest no longer compares equal to the old empty aggregate sentinel.
	...
	if got := res.NodeOutputs["stage/bare.then"]; got != "" {
		t.Errorf("bare-value then output = %q, want empty (scatter value is its harvest)", got)
	}
}
...
func TestGuardInScatterReadsFailedSiblingOutcomeRoot(t *testing.T) {
	...
	// Structural scatter reports a successful harvest; the failed sibling
	// remains visible in that harvest and to the guard above.
	if settled["lanes"] != engine.OutcomeSucceeded {
		t.Errorf("scatter aggregate lanes = %q, want succeeded", settled["lanes"])
	}
...
func TestGuardInScatterReadsFailedSiblingOutcomeNS(t *testing.T) {
	...
	if settled["stage/lanes"] != engine.OutcomeSucceeded {
		t.Errorf("sub-scatter stage/lanes = %q, want succeeded", settled["stage/lanes"])
	}
	if settled["stage"] != engine.OutcomeSucceeded {
		t.Errorf("run aggregate stage = %q, want succeeded", settled["stage"])
	}
	if res.Outcome != engine.OutcomeSucceeded {
		t.Errorf("run outcome = %q, want succeeded", res.Outcome)
	}
...
func TestAdvanceGuardWriteOnceNoReEvalOnGrownFold(t *testing.T) {
	...
	// A journal row not present in the formula cannot alter its source-defined
	// terminal result.
	if r4.Run.Outcome != engine.OutcomeSucceeded {
		t.Errorf("run outcome = %q, want succeeded formula result", r4.Run.Outcome)
	}
}
