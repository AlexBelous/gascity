...
const (
	...
	dependencySkippedDetail = "skipped: upstream dependency did not pass"
)
...
const (
	...
	//
	// TYPED RESULTS — v7. outcome.settled and run.closed now carry the upstream
	// LumenStepResult alongside the legacy string outcome/output fields. The
	// reducer folds those values into node and run state so snapshots preserve
	// exact structured values across crash/resume. Resume migrates v6 snapshots
	// by deriving the best lossless result available from their legacy fields.
	reducerVersion = 7
...
type runStartedPayload struct {
	...
	// loop drives it; "self" = one agent session drives it turn-by-turn in
...
// never a Go inference. It is additive and omitempty — a non-retryable settle
// omits only that classification field.
type outcomeSettledPayload struct {
	Activation       string           `json:"activation"`
	Outcome          string           `json:"outcome"`
	Output           string           `json:"output,omitempty"`
	Result           *LumenStepResult `json:"result,omitempty"`
