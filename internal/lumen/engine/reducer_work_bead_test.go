...
func TestReducerVersionBumpStrandsOldSnapshot(t *testing.T) {
	...
	if !errors.Is(err, fold.ErrReducerVersionSkew) {
		t.Fatalf("Fold of a v3 snapshot under the v7 reducer = %v, want ErrReducerVersionSkew", err)
	}
	if got := Reducer().ReducerVersion(); got != 7 {
		t.Fatalf("ReducerVersion() = %d, want 7", got)
	}
}
