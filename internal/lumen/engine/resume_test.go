...
func TestResumeDetectsCorruptedSnapshot(t *testing.T) {
	...
	tampered[0] ^= 0xff
	tx, err := store.ReadDB().BeginTx(ctx, nil)
	if err != nil {
...
func TestSnapshotDisabledIsInertAndFoldCompatible(t *testing.T) {
	...
	var snapCount int
	if err := store.ReadDB().QueryRowContext(ctx, `SELECT COUNT(*) FROM snapshots`).Scan(&snapCount); err != nil {
...
func TestResumeReconcilesProjectionForSealedRun(t *testing.T) {
	...
	var nodeRows int
	if err := store.ReadDB().QueryRowContext(ctx, `SELECT COUNT(*) FROM nodes WHERE stream_id = ?`, stream).Scan(&nodeRows); err != nil {
	...
	if err := store.ReadDB().QueryRowContext(ctx, `SELECT COUNT(*) FROM frontier WHERE root_id = ?`, stream).Scan(&frontierRows); err != nil {
