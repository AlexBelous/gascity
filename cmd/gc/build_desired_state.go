...
func buildDesiredStateWithSessionBeads(
	...
	for i := range cfg.Agents {
		...
		if store != nil && !hasCustomScaleCheck {
			...
			// scope) at the pointer level; distinct Store values over the same
			// backing are additionally handled by the cross-group bead-ID dedup
			// in defaultScaleCheckCountsAndDemand.
			if ownTarget.storeKey != "city" && ownTarget.store != nil && ownTarget.err == nil && ownTarget.store != store {
