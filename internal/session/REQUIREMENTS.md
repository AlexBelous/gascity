...
| SESSION-RECON-014 | Atomic fresh pool runtime-name reservation | A fresh pool create derives its exact runtime name before mutation, locks that name together with every alias/identity spelling it checks in deterministic city scope, then rechecks every configured session-census store with an authoritative live/cache-bypassing read before creating and unlocking. Exact-name collisions (including a foreign holder arriving after the planning census or hidden by a stale production cache), unanswerable/partial reservation queries, and lock failures create no bead or primary writeback. Proven public-alias collisions in either the primary or a foreign session store retain the explicit alias-deferral path. | `cmd/gc/session_name_lookup.go`; `cmd/gc/build_desired_state.go`; `cmd/gc/pool_session_identifier_lock_test.go` (`TestCreatePoolSessionBeadWithGuardedAlias_TransientExactNameRace`, `TestCreatePoolSessionBeadWithGuardedAlias_AliasedSlot2ExactNameRace`, `TestCreatePoolSessionBeadWithGuardedAlias_AliasAvailabilityErrorNeverCreates`, `TestCreatePoolSessionBeadWithGuardedAlias_ProvenAliasCollisionDefersAlias`, `TestCreatePoolSessionBeadWithGuardedAlias_ForeignAliasCollisionDefersAlias`, `TestCreatePoolSessionBeadWithGuardedAlias_LateForeignExactNameHolderBlocksCreate`, `TestCreatePoolSessionBeadWithGuardedAlias_LiveRecensusBypassesStaleForeignCache`, `TestCreatePoolSessionBeadWithGuardedAlias_ForeignRecensusErrorNeverCreates`, `TestCreatePoolSessionBeadWithIdentifiers_UsesForeignAvailabilitySnapshotAndPrimaryWriteback`); `cmd/gc/build_desired_state_test.go` (`TestCreatePoolSessionBeadWithGuardedAliasFailsClosedWhenIdentifierLockFails`); `cmd/gc/pool_session_name_holder_test.go` (`TestPoolSessionCreate_DegradedStoreRefusesWithoutALiveHolder`, `TestPoolSessionCreate_DegradedTransientIdentityCheckRefusesWithoutCreating`) |
| SESSION-RECON-015 | City suspension is a hard wake barrier | Effective city suspension defeats every wake and retention cause, including pending create, explicit wake, pin, named/config demand, assigned work, attachment, pending interaction, and ready wait. The start executor re-reads suspension immediately before provider start so a suspend that lands after planning still cancels the launch. That cancellation is not a provider failure: it releases only the transient start lease and preserves durable wake intent for a later resume. Repeated startup/reload reconcile passes produce no provider start. | `cmd/gc/compute_awake_set_test.go` (`TestPendingCreate_EffectivelySuspendedAgentDoesNotWake`, `TestComputeAwakeSet_CitySuspendedOverridesEveryWakeCause`); `cmd/gc/compute_awake_bridge_test.go` (`TestBuildAwakeInputFromReconcilerCarriesEffectiveCitySuspension`); `cmd/gc/session_lifecycle_start_boundary_test.go` (`TestRunPreparedStartCandidate_CitySuspendedAfterPreparationSkipsProviderStart`); `cmd/gc/session_reconciler_test.go` (`TestReconcileSessionBeads_SuspendedCityNeverStartsPendingCreateAcrossRestartPasses`); `gc-f3xbhs` |
...
| SESSION-WORK-005 | Late claim cancels queued asynchronous stop | Before an asynchronous provider stop, a reconciler-owned no-wake or orphan drain leaves a bounded settle window for an in-flight claim to commit, rechecks the runtime token, then rechecks every work store reachable by that session. Newly claimed in-progress or ready open work restores the still-running stop-pending session to active and clears the reconciler drain. An unanswerable work query or failed durable cancellation fails closed and does not stop the runtime. Explicit agent-authored drain acknowledgements retain their prompt stop semantics and do not enter the settle window. | `cmd/gc/session_reconciler.go`; `cmd/gc/session_reconciler_test.go` (`TestQueueDrainAckAsyncStopCancelsForLateRigClaim`, `TestQueueDrainAckAsyncStopSettlesBeforeFinalLateClaimCheck`, `TestQueueDrainAckAsyncStopRechecksTokenAfterLateClaimSettle`, `TestQueueDrainAckAsyncStopFailsClosedWhenLateClaimQueryFails`, `TestQueueDrainAckAsyncStopFailsClosedWhenLateClaimCancellationWriteFails`, `TestQueueDrainAckAsyncStopPreservesAgentAuthoredStopSemantics`); `internal/session/store_test.go` (`TestCancelDrainAckStopPendingEmitsSingleUpdateAndReturnsActiveInfo`); `internal/session/lifecycle_transition_test.go` |
