...
func TestSkippedOrderingDependencyIsBenign(t *testing.T) {
	...
	if res.Outcome != engine.OutcomeSucceeded {
		t.Errorf("run outcome = %q, want succeeded (last non-skipped step C wins)", res.Outcome)
	}
	settled := settledOutcomeByID(t, res.Events)
...
func dumpTierA(t *testing.T, store *graphstore.Store, stream string) string {
	...
	_ = rows.Close()
	mrows, err := store.ReadDB().QueryContext(ctx,
	...
	_ = mrows.Close()
	erows, err := store.ReadDB().QueryContext(ctx,
	...
	_ = erows.Close()
	frows, err := store.ReadDB().QueryContext(ctx,
