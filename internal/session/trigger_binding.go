...
func (s *Store) RebindTriggerIfMatch(info Info, persisted PersistedResponse, binding TriggerBinding) (Info, error) {
	if s == nil || s.store.Store == nil {
		return info, fmt.Errorf("rebinding trigger for %q: session store is unavailable", info.ID)
	}
	binding = binding.normalized()
	if err := binding.validate(); err != nil {
		return info, fmt.Errorf("rebinding trigger for %q: %w", info.ID, err)
	}
	...
	if !triggerPreimageMatchesPersisted(info, persisted.Metadata) {
		return info, fmt.Errorf("rebinding trigger for %q: Info and persisted response are not the same preimage", info.ID)
	}
	...
	if err != nil {
		return info, fmt.Errorf("rebinding trigger for %q: resolving conditional writer: %w", info.ID, err)
	}
	if writer == nil {
		return info, fmt.Errorf("rebinding trigger for %q: %w", info.ID, beads.ErrConditionalWriteUnsupported)
	}
