...
// sweep WITHOUT reading it — the same bounded discovery SweepSessionModelUsage runs
// internally, exposed so a caller can memoize the resolved path across ticks. The
// live-session sweep uses this to resolve each transcript once instead of re-scanning
// codex day dirs every reconcile tick (the sweep Factory is rebuilt per tick, so the
// memo lives on the caller). It preserves BOTH discovery routes of
// discoverSweepTranscript — the keyed rollout lookup AND the ambiguity-guarded
// keyless (cwd, wake-window) workdir fallback — so a KEYLESS codex wisp still resolves
// its rollout. Returns "" when the family is unregistered or no rollout is discovered
// yet (rollout not flushed, out of window, or an ambiguous/clouded keyless scan); the
// caller treats "" as "retry next tick" and does not cache it. The scanClean
// distinction (settle vs transient) is immaterial here: the live arm has no
// per-interval settle marker and re-sweeps every tick regardless.
func (f *Factory) DiscoverSweepTranscript(id string, meta map[string]string, now time.Time) string {
	...
	path, _ := f.discoverSweepTranscript(family, id, meta, now)
	return path
}

// SweepSessionModelUsageAtPath is SweepSessionModelUsage against an ALREADY-RESOLVED
// transcript path — the seam the live-session sweep uses so a memoized rollout path
// skips per-tick discovery. It performs the same family gate, cursor-guarded
// extraction, per-invocation fact/metric emission, cursor advance, and settle/err
// contract as SweepSessionModelUsage; ONLY the discovery step is replaced by the
// supplied path. An empty path is a transient miss (settled=false). Every other
// property — exactly-once via the shared invocation-usage cursor +
// usage.ModelIdempotencyKey, partial-tail safety, the 64KB tail bound, and the
// sink-failure cursor semantics — is identical, because both entry points delegate to
// sweepResolvedTranscript.
