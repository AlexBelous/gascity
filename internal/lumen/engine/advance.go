...
func Advance(ctx context.Context, store *graphstore.Store, doc *ir.IR, streamID string, input map[string]any, opts Options) (AdvanceResult, error) {
	...
	if d.st().Closed {
		...
		result, err := d.completedResult(scope)
		if err != nil {
			return AdvanceResult{}, fmt.Errorf("lumen: advance %q: completed result: %w", streamID, err)
		}
		return AdvanceResult{
			Sealed: true,
			Run: RunResult{
				StreamID:    streamID,
				Outcome:     d.st().Outcome,
				Result:      result,
	...
	if d.allUnitsSettled(units) {
		...
		if err != nil {
			return AdvanceResult{}, fmt.Errorf("lumen: advance %q: formula result: %w", streamID, err)
		}
...
// settle, then returns their exact results in an indexed successful harvest. An empty
// array returns an empty harvest in one pass. It is re-entrant: re-mint is write-once
// (a member already in the fold is skipped) and the array is DET-stable, so a re-Advance
// with no new settlement converges.
...
// settleForEach returns a successful indexed harvest containing each settled
// member's exact result. Failed and skipped members are values, not fan failures.
func (d *driver) settleForEach(
	u planUnit,
	elements []string,
	scope, nodeOutputs map[string]string,
) error {
	result, err := d.forEachStepResult(u, elements, nil, scope)
	if err != nil {
		return fmt.Errorf("lumen: for-each %q result: %w", u.nodeID, err)
	}
	output, err := legacyOutput(result.Value)
	if err != nil {
		return fmt.Errorf("lumen: for-each %q output: %w", u.nodeID, err)
	}
...
// it remains data in the complete indexed harvest returned after every member settles.
//
