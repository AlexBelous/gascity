...
// TestBeadsWorkRemoteConfigParseTable pins the [beads.work] remote_config field:
// empty/write/verify with a remote target parse and resolve; an unknown value or a
// non-empty value without a remote target fail load.
func TestBeadsWorkRemoteConfigParseTable(t *testing.T) {
	base := func(remoteConfig string, withTarget bool) string {
		target := ""
		if withTarget {
			target = "scope = \"unified\"\ntarget = \"dolt://db.example.com:3306/org\"\n"
		}
		rc := ""
		if remoteConfig != "" {
			rc = "remote_config = \"" + remoteConfig + "\"\n"
		}
		return "[workspace]\nname = \"test\"\n\n[beads.work]\n" + target + rc
	}

	t.Run("verify with a remote target parses and resolves", func(t *testing.T) {
		cfg, err := Parse([]byte(base("verify", true)))
		if err != nil {
			t.Fatalf("verify+remote rejected: %v", err)
		}
		if !cfg.Beads.Work.RemoteConfigIsVerify() {
			t.Fatal("remote_config=verify must report RemoteConfigIsVerify()")
		}
		if got := cfg.Beads.Work.EffectiveRemoteConfig(); got != BeadsWorkRemoteConfigVerify {
			t.Fatalf("EffectiveRemoteConfig() = %q, want verify", got)
		}
	})

	t.Run("write with a remote target parses as not-verify", func(t *testing.T) {
		cfg, err := Parse([]byte(base("write", true)))
		if err != nil {
			t.Fatalf("write+remote rejected: %v", err)
		}
		if cfg.Beads.Work.RemoteConfigIsVerify() {
			t.Fatal("remote_config=write must NOT be verify")
		}
	})

	t.Run("empty defaults to write", func(t *testing.T) {
		cfg, err := Parse([]byte(base("", true)))
		if err != nil {
			t.Fatalf("empty remote_config rejected: %v", err)
		}
		if cfg.Beads.Work.RemoteConfigIsVerify() || cfg.Beads.Work.EffectiveRemoteConfig() != BeadsWorkRemoteConfigWrite {
			t.Fatalf("empty remote_config must default to write, got verify=%v effective=%q", cfg.Beads.Work.RemoteConfigIsVerify(), cfg.Beads.Work.EffectiveRemoteConfig())
		}
	})

	t.Run("unknown value fails load", func(t *testing.T) {
		_, err := Parse([]byte(base("gateway", true)))
		if err == nil || !strings.Contains(err.Error(), "beads.work.remote_config") || !strings.Contains(err.Error(), "gateway") {
			t.Fatalf("unknown remote_config must fail load naming the field + value, got %v", err)
		}
	})

	t.Run("set without a remote target fails load", func(t *testing.T) {
		_, err := Parse([]byte(base("verify", false)))
		if err == nil || !strings.Contains(err.Error(), "remote_config") || !strings.Contains(err.Error(), "remote") {
			t.Fatalf("remote_config without a remote target must fail load, got %v", err)
		}
	})
}
