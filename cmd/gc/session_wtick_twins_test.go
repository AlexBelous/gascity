...
func TestRetireDuplicateRowsMatchesBeads(t *testing.T) {
	...
	runRaw := func(store beads.Store, sp *runtime.Fake) {
		...
		retireDuplicateConfiguredNamedSessionBeads(store, nil, nil, sp, cfg, cityName, rawBeads, bySessionName, indexBySessionName, now, nil)
	}
	runRows := func(store beads.Store, sp *runtime.Fake) {
		...
		retireDuplicateConfiguredNamedSessionRows(store, nil, nil, sp, cfg, cityName, rows, now, nil)
	}
