	...
	for i := range cfg.Agents {
		...
		if backsNamedSession {
			...
			if store != nil && !hasCustomScaleCheck {
				...
				// SPLIT-STORE CONTROL DISPATCHER (fork): un-gated, mirroring the
				// generic-pool probe below. The fork serve-loop sweep lets a rig
				// control dispatcher claim city-store routed control work, so the
				// named-backing pool probe must surface that city-store demand too.
