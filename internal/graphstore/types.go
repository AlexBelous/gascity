...
var (
	...
	// ErrSnapshotHashMismatch is returned when a persisted or proposed
	// snapshot's state bytes do not match its recorded canonical hash.
	ErrSnapshotHashMismatch = errors.New("graphstore: snapshot state hash mismatch")

	// ErrNoCoveringSnapshot is returned when retention or a truncated-stream
	// rebuild cannot find the durable snapshot that owns the missing prefix.
	ErrNoCoveringSnapshot = errors.New("graphstore: no covering snapshot")
)
