...
func TestResolveDeclaredInput(t *testing.T) {
	...
	// An EXPLICIT null in raw stays null (bound-to-null), rather than being replaced
	// by the field default.
	...
	// A zero-value Field with no presence bit represents an omitted default.
	if _, err := resolveDeclaredInput([]ir.Field{{Name: "y", Required: true, Default: nil}}, nil); !errors.Is(err, ErrRequiredInputUnbound) {
		t.Errorf("required field with omitted default err = %v, want ErrRequiredInputUnbound", err)
	}
	// An explicitly authored null default is present and valid for a null field.
	if _, err := resolveDeclaredInput([]ir.Field{{
		Name:       "y",
		Type:       ir.Type{Kind: ir.TypeAtomic, Name: "null"},
		Required:   true,
		Default:    nil,
		HasDefault: true,
	}}, nil); err != nil {
		t.Errorf("required field with explicit default:null: %v", err)
	}
}
