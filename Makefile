...
dashboard-check: dashboard-build
	...
	@packages="$$(go list ./internal/api/dashboardspa/...)" || exit $$?; \
	if printf '%s\n' "$$packages" | grep -q '/node_modules/'; then \
		echo "ERROR: dashboard node_modules leaked into the parent Go module package graph" >&2; \
		printf '%s\n' "$$packages" | grep '/node_modules/' >&2; \
		exit 1; \
	fi
