...
// TestForEachRunBodyDownstreamReads pins the three downstream consumers of a
// run-bodied fan: its legacy harvest JSON is renderable, its successful outcome
// is visible to guards, and the element binder is restored after fan-out.
func TestForEachRunBodyDownstreamReads(t *testing.T) {
	...
	// (a) template-form {{fan}} renders the complete indexed harvest.
	wantHarvest := `v={"0":{"value":"item=a target=t","outcome":"succeeded","error":null},"1":{"value":"item=b target=t","outcome":"succeeded","error":null}}`
	if got := res.NodeOutputs["probe"]; got != wantHarvest {
		t.Errorf("downstream probe = %q, want complete harvest %q", got, wantHarvest)
	}
...
// TestForEachRunBodyAggregateVisibilityInNs proves the same result, outcome, and
// dependency visibility inside a run namespace.
func TestForEachRunBodyAggregateVisibilityInNs(t *testing.T) {
	...
	wantHarvest := `v={"0":{"value":"item=a target=L","outcome":"succeeded","error":null}}`
	if got := res.NodeOutputs["stage/probe"]; got != wantHarvest {
		t.Errorf("ns render probe = %q, want complete harvest %q", got, wantHarvest)
	}
}
...
// TestForEachRunBodyFailureDrainContinue proves a failed member run remains exact
// data in the successful fan harvest and does not suppress sibling execution.
func TestForEachRunBodyFailureDrainContinue(t *testing.T) {
	...
	if settled["fan"] != engine.OutcomePass {
		t.Errorf("fan = %q, want pass (member failure is harvest data)", settled["fan"])
	}
	...
	requireResultJSON(t, res.Result, `{
		"value":{
			"0":{"value":"ok ok","outcome":"succeeded","error":null},
...
// TestForEachRunBodyFailureDrainStop proves the legacy on_fail field does not
// discard a failed member or suppress later run members.
...
// (no error, no suppression) → member 1 closes pass → the fan returns both results and
// seals. The mutant this kills: an added break/return-on-first-failed-agg
...
func TestAdvanceForEachRunBodyFailedMemberNoSuppression(t *testing.T) {
	...
	// Member 1 passes → pass 3 observes it, returns the complete harvest, and seals.
	...
	if settled["fan"] != engine.OutcomePass {
		t.Errorf("fan = %q, want pass (failed member preserved as harvest data)", settled["fan"])
	}
	requireResultJSON(t, r3.Run.Result, `{
		"value":{
			"0":{"value":null,"outcome":"failed","error":{"reason":"failed","retryable":true}},
			"1":{"value":"ok","outcome":"succeeded","error":null}
		},
		"outcome":"succeeded",
		"error":null
	}`)
...
// TestForEachRunBodyFailureDrainInsideNs proves both legacy on_fail spellings
// preserve failed member data in a successful harvest inside a run namespace.
	...
	for _, tc := range []struct {
		...
		t.Run(tc.onFail, func(t *testing.T) {
			...
			if settled["stage/fan"] != engine.OutcomePass {
				t.Errorf("ns fan = %q, want pass (on_fail %s)", settled["stage/fan"], tc.onFail)
			}
			if settled["stage"] != engine.OutcomePass {
				t.Errorf("enclosing run stage = %q, want pass (transparent harvest)", settled["stage"])
			}
		})
	}
}
...
// host — exactly 2 calls total across crash+resume), member 0's aggregate remains failed
// data, member 1 runs fresh, and the fan returns the complete successful harvest.
