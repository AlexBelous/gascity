...
| `work_query_federated` | boolean |  |  | WorkQueryFederated declares that the custom WorkQuery reads every reachable work store, including relocated coordination classes and ephemeral work. gc hook runs it once in the primary query environment instead of once per store. False preserves legacy custom-query fan-out. Ignored without WorkQuery. |
...
| `work_query_federated` | boolean |  |  | WorkQueryFederated overrides the custom work-query coverage declaration. |
