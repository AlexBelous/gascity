...
type RunResult struct {
	...
	Outcome string
	// Result is the formula's upstream-conformant structured result.
...
func RunWithOptions(ctx context.Context, store *graphstore.Store, doc *ir.IR, input map[string]any, opts Options) (RunResult, error) {
	...
	formulaResult, err := d.formulaResult(scope)
	if err != nil {
		return RunResult{}, fmt.Errorf("lumen: formula result: %w", err)
	}
	runOutcome := d.outcomeForDialect(formulaResult.Outcome)
	...
	if err := d.append(EventRunClosed, streamID+":run:closed", runClosedPayload{
		Outcome: runOutcome,
	...
	completedResult, err := d.completedResult(scope)
	if err != nil {
		return RunResult{}, fmt.Errorf("lumen: completed result: %w", err)
	}
	return RunResult{
		StreamID:    streamID,
		Outcome:     runOutcome,
...
type driver struct {
	...
	host     enginehost.AgentHost
	doc      *ir.IR
	// units is the formula's lowered, topologically ordered plan. Resume uses it
	// to recover exact typed results from pre-v7 settlements whose journal rows
	// carry only legacy string output.
	units []planUnit
...
func (d *driver) runUnit(u planUnit, scope, nodeOutputs map[string]string) error {
	...
	if d.blocked(u) {
		if err := d.appendSettled(u.activation, OutcomeSkipped, "", dependencySkippedDetail); err != nil {
			return err
		}
	...
	case unitScatterAgg:
		runErr = d.runScatter(u, scope, nodeOutputs)
	case unitGather:
	...
	case unitCleanupGuarded:
		...
		runErr = d.runCleanupGuarded(u, scope, nodeOutputs)
	case unitRecover:
...
// aggregateAllSkipped reports whether a non-scatter drain aggregate's every
// member settled skipped/canceled. Structural scatter is excluded because its
// result is always a successful harvest, with skipped members preserved as
// data. A unit with no drain members is never "all skipped".
func (d *driver) aggregateAllSkipped(u planUnit) bool {
	if u.kind == unitScatterAgg {
		return false
	}
...
func (d *driver) runLeaf(u planUnit, scope, nodeOutputs map[string]string) error {
	...
	case ir.NodeExec:
		...
		stdout, stderr, exitCode, runErr := exechost.Run(d.ctx, u.leaf.program, script, u.leaf.cwd, u.leaf.env)
		if runErr != nil {
		...
		result := runtimeStepResult(outcome, resultValue, reason, retryable, nil)
		if result.Error != nil {
			result.Error.Message = strings.TrimRight(stderr, "\n")
		}
		if err := d.appendOutcomeSettled(outcomeSettledPayload{
	...
	case ir.NodeSettle:
		...
		if raw, ok := u.leaf.raw["value"]; ok {
			resultContext, err := d.resultContextFor(u.ns, scope)
			if err != nil {
				return fmt.Errorf("lumen: settle %q context: %w", u.nodeID, err)
			}
			structured, err = evaluateLumenExpr(raw, resultContext)
			if err != nil {
				return fmt.Errorf("lumen: settle %q value: %w", u.nodeID, err)
			}
			value, err = legacyOutput(structured)
			if err != nil {
				return fmt.Errorf("lumen: settle %q output: %w", u.nodeID, err)
			}
		}
		...
		result := authoredSettleResult(outcome, structured, reason)
		if u.leaf.publicOutcome {
			result = publicOutcomeResult(outcome, structured, reason, nil)
		}
		if err := d.appendOutcomeSettled(outcomeSettledPayload{
			Activation: u.activation,
			Outcome:    outcome,
...
func (d *driver) runDo(u planUnit, scope, nodeOutputs map[string]string) error {
	...
	hostResult, runErr := d.host.RunDo(d.ctx, enginehost.DoRequest{
		RunID:      d.streamID,
		NodeID:     u.nodeID,
		Activation: u.activation,
		IdemToken:  effectIdem,
	...
	nodeOutcome, effResult, detail, out, session := foldDoResult(hostResult, runErr)
	...
	resultValue := any(out)
	if nodeOutcome == OutcomeFailed || nodeOutcome == OutcomeCanceled {
		resultValue = nil
	}
	result := runtimeStepResult(nodeOutcome, resultValue, detail, false, nil)
	if result.Error != nil {
...
// runScatter settles a structural scatter as a successful harvest. Individual
// member failures remain visible as LumenStepResult values in that harvest;
// they are data rather than a failure of the fan itself.
func (d *driver) runScatter(u planUnit, scope, nodeOutputs map[string]string) error {
	context, err := d.resultContextFor(u.ns, scope)
	if err != nil {
		return fmt.Errorf("lumen: scatter %q context: %w", u.nodeID, err)
	}
	...
	if err != nil {
		return fmt.Errorf("lumen: scatter %q result: %w", u.nodeID, err)
	}
	output, err := legacyOutput(result.Value)
	if err != nil {
		return fmt.Errorf("lumen: scatter %q output: %w", u.nodeID, err)
	}
	if err := d.appendOutcomeSettled(outcomeSettledPayload{
		Activation: u.activation,
...
// way the aggregate returns an indexed harvest containing every member's exact result;
// failed and skipped members remain data, so the fan itself succeeds. An empty/absent
// array returns an empty harvest; a non-array `over` settles failed{invalid_input}. It
// is reached only when the for-each is not gated off — runUnit handles
// blocked()/skip-cascade before this switch arm.
func (d *driver) runForEach(u planUnit, scope, nodeOutputs map[string]string) error {
	...
	return d.settleForEach(u, elems, scope, nodeOutputs)
}
...
func (d *driver) runForEachRunMember(u planUnit, index int, scope, nodeOutputs map[string]string) error {
	...
	// body lowering or host is broken — surface it loudly rather than synthesize a
	// misleading harvest entry.
...
func (d *driver) cleanupGuardedResultUnit(u planUnit) planUnit {
	if u.cleanup.guardedAgg == "" {
		return d.cleanupGuardedUnit(u)
	}
	return planUnit{
		kind:         unitCleanupGuarded,
		activation:   u.cleanup.guardedAgg,
		nodeID:       activationNodeID(u.cleanup.guardedAgg),
		irKind:       ir.NodeBlock,
		ns:           u.ns,
		resultNodes:  u.cleanup.guardedNodes,
		resultPrefix: u.ns,
	}
}
...
// settleCleanup preserves the guarded result after the finalizer settles. Legacy
// journals retain their historical finalizer-failure precedence during replay.
func (d *driver) settleCleanup(u, gu, bu planUnit, scope, nodeOutputs map[string]string) error {
	...
	guarded := st.Nodes[guardedAct]
	if st.SemanticDialect != SemanticDialectLegacy {
		if err := d.appendTransparentResult(u.activation, d.cleanupGuardedResultUnit(u), scope); err != nil {
			return err
		}
		if ranOutcome(guarded.Outcome) {
			d.record(u.nodeID, guarded.Output, scope, nodeOutputs)
		}
		return nil
	}

	guardedOutcome, guardedOutput := OutcomeSkipped, ""
	if guarded != nil && guarded.Settled {
		guardedOutcome, guardedOutput = guarded.Outcome, guarded.Output
	}
...
func (d *driver) settleRecoverFrom(u, su planUnit, scope, nodeOutputs map[string]string) error {
	source := d.st().Nodes[su.activation]
	if err := d.appendTransparentResult(u.activation, su, scope); err != nil {
		return err
	}
	if ranOutcome(source.Outcome) {
		d.record(u.nodeID, source.Output, scope, nodeOutputs)
	}
	return nil
}
...
func (d *driver) runRun(u planUnit, scope, nodeOutputs map[string]string) error {
	output, err := d.settleTransparentAgg(u, scope)
	if err != nil {
		return err
	}
	d.record(u.nodeID, output, scope, nodeOutputs)
...
// settleTransparentAgg settles a run or cleanup block with the same source-order
// accumulator as an upstream sequential block. It records nothing into scope;
// callers decide where the aggregate's legacy output is visible.
func (d *driver) settleTransparentAgg(u planUnit, scope map[string]string) (output string, err error) {
	st := d.st()
	var result LumenStepResult
	if len(u.resultNodes) > 0 {
		result, err = d.authoredUnitResult(u, scope)
		if err != nil {
			return "", fmt.Errorf("lumen: aggregate %q result: %w", u.nodeID, err)
		}
	} else {
		acc := newBlockResult(OutcomeSucceeded)
		for _, member := range u.members {
			if node := st.Nodes[member]; node != nil && node.Settled {
				result, source := resultFromNode(node)
				if source.available() {
					acc.add(result)
				} else {
					acc.addUnavailable(node.Outcome)
				}
			}
		}
		result, err = acc.finish()
		if err != nil {
			return "", fmt.Errorf("lumen: aggregate %q result: %w", u.nodeID, err)
		}
	}
	output, err = legacyOutput(result.Value)
	if err != nil {
		return "", fmt.Errorf("lumen: aggregate %q output: %w", u.nodeID, err)
	}
	if err := d.appendOutcomeSettled(outcomeSettledPayload{
		Activation: u.activation,
		Outcome:    result.Outcome,
...
func (d *driver) runCleanupGuarded(u planUnit, scope, nodeOutputs map[string]string) error {
	output, err := d.settleTransparentAgg(u, scope)
	if err != nil {
		return err
	}
...
func (d *driver) settleDecisionFromBody(u, bu planUnit, scope, nodeOutputs map[string]string) error {
	body := d.st().Nodes[bu.activation]
	if err := d.appendTransparentResult(u.activation, bu, scope); err != nil {
		return err
	}
	d.record(u.nodeID, body.Output, scope, nodeOutputs)
	return nil
}
...
func (d *driver) condScope(ns string, scope, nodeOutputs map[string]string) (loopScope, error) {
	...
	for _, fld := range parts.spec.inputFields {
		if !bound[fld.Name] && fieldHasDefault(fld) {
			backfill(fld.Name)
		}
	}
	return loopScope{
...
// ErrInvalidInput is returned before run.started when a provided or defaulted
// formula input does not match its declared Lumen type.
var ErrInvalidInput = errors.New("lumen: invalid input")
...
// layer, and a required field with neither a provided value nor an authored
// default additionally returns ErrRequiredInputUnbound naming the field(s).
// Explicit default:null is an authored default and remains distinct from an
// omitted default. The three genesis surfaces consume the error and refuse before
// any journal append; typedSubInput and rebuildDriver DISCARD it and keep the
// tolerant present-null layer.
func resolveDeclaredInput(fields []ir.Field, raw map[string]any) (map[string]any, error) {
	...
	for _, f := range fields {
		if v, ok := raw[f.Name]; ok {
			out[f.Name] = v
			if !matchesInputType(v, f.Type) {
				invalid = append(invalid, strconv.Quote(f.Name))
			}
			continue
		}
		if fieldHasDefault(f) {
			out[f.Name] = f.Default
			if !matchesInputType(f.Default, f.Type) {
				invalid = append(invalid, strconv.Quote(f.Name))
			}
			continue
		}
	...
	if len(invalid) > 0 {
		return out, fmt.Errorf("%w: %s (value does not match declared type)", ErrInvalidInput, strings.Join(invalid, ", "))
	}
	if len(missing) > 0 {
		return out, fmt.Errorf("%w: %w: %s (no value provided, no default)",
			ErrInvalidInput, ErrRequiredInputUnbound, strings.Join(missing, ", "))
	}
	return out, nil
}

func fieldHasDefault(field ir.Field) bool {
	return field.HasDefault || field.Default != nil
}

func matchesInputType(value any, typ ir.Type) bool {
	switch typ.Kind {
	case ir.TypeAlias:
		return typ.Target != nil && matchesInputType(value, *typ.Target)
	case ir.TypeLiteral:
		return reflect.DeepEqual(value, typ.Value)
	case ir.TypeUnion:
		for _, candidate := range typ.Of {
			if matchesInputType(value, candidate) {
				return true
			}
		}
		return false
	case ir.TypeArray:
		...
		for _, item := range items {
			if !matchesInputType(item, *typ.Element) {
				return false
			}
		}
		return true
	case ir.TypeRecord:
		...
		for _, field := range typ.Fields {
			...
			item, present := record[field.Name]
			if !present {
				if field.Required {
					return false
				}
			...
			if !matchesInputType(item, field.Type) {
				return false
			}
		}
		if typ.AdditionalFields != nil {
			for name, item := range record {
				if !declared[name] && !matchesInputType(item, *typ.AdditionalFields) {
					return false
				}
			}
		}
	...
	case ir.TypeChannel:
		...
		if !ok || channel["kind"] != "ChannelCapability" {
			return false
		}
		capability, ok := channel["capability"].(string)
		if !ok {
			return false
		}
		required := string(typ.Capability)
		return capability == required || capability == "both"
	case ir.TypeHandle:
		handle, ok := value.(map[string]any)
		return ok && handle["kind"] == "handle" && handle["type"] == typ.Name
	case ir.TypeAtomic:
		switch typ.Name {
		case "string", "path", "duration", "quote":
			_, ok := value.(string)
			return ok
		case "number":
		...
		case "outcome":
			...
			return ok && knownOutcome(outcome)
		default:
			return true
		}
	default:
...
func isInputNumber(value any) bool {
	switch value.(type) {
...
func knownOutcome(outcome string) bool {
	switch outcome {
	case OutcomeSucceeded, outcomeLegacyPass, OutcomeDegraded, OutcomeFailed, OutcomeSkipped, OutcomeCanceled:
		return true
	default:
		return false
	}
...
func (d *driver) runGather(u planUnit, scope, nodeOutputs map[string]string) error {
	...
	context, err := d.resultContextFor(u.ns, scope)
	if err != nil {
		return fmt.Errorf("lumen: gather %q context: %w", u.nodeID, err)
	}
	result, err := blockResult(
		u.resultNodes,
		u.resultPrefix,
		d.st(),
		context,
	...
	if err != nil {
		return fmt.Errorf("lumen: gather %q result: %w", u.nodeID, err)
	}
	output, err := legacyOutput(result.Value)
	if err != nil {
		return fmt.Errorf("lumen: gather %q output: %w", u.nodeID, err)
	}
	if err := d.appendOutcomeSettled(outcomeSettledPayload{
		Activation: u.activation,
		Outcome:    result.Outcome,
...
func (d *driver) runLoop(u planUnit, scope, nodeOutputs map[string]string) error {
	...
	if spec.irKind == ir.NodeRetry {
		...
		if !ok {
			return d.settleLoop(u, 0, nil, "invalid_input", nil, 0, scope, nodeOutputs)
		}
		maxAttempts = n
	}
...
func (d *driver) loopDecide(u planUnit, attempt, maxAttempts int, bn *nodeState, scope, nodeOutputs map[string]string) (bool, error) {
	...
	case ir.NodeRepeat:
		...
		if truthy {
			...
			return false, d.settleLoop(u, attempt, bn, "", nil, 0, scope, nodeOutputs)
		}
		...
		if bn.Outcome == OutcomePending && pendingPolls >= lumenLoopPhysicalCap {
			return false, d.settleLoop(u, attempt, bn, "poll_cap", nil, 0, scope, nodeOutputs)
		}
		if iteration >= d.repeatLimit {
			return false, d.settleLoop(u, attempt, bn, "loop_cap", nil, 0, scope, nodeOutputs)
		}
		return true, nil
	...
	case ir.NodeRetry:
		...
		if !bn.Retryable {
			...
			rem := maxAttempts - n
			return false, d.settleLoop(u, attempt, bn, "", &rem, 0, scope, nodeOutputs)
		}
		if n >= maxAttempts {
			// The budget is exhausted by retryable failures.
			rem := 0
			return false, d.settleLoop(u, attempt, bn, "exhausted", &rem, maxAttempts, scope, nodeOutputs)
		}
		return true, nil
...
func (d *driver) settleLoop(
	...
	attempts int,
	scope, nodeOutputs map[string]string,
) error {
	var sourceResult *LumenStepResult
	if source != nil {
		sourceUnit := d.attemptUnit(u, attempt)
		if u.loop.bodyRun != nil {
			subUnits, aggregate, err := u.loop.mintRunBodyAttempt(
				attempt,
				u.activation,
				u.ns,
				u.afterDeps,
		...
		if err != nil {
			return fmt.Errorf("lumen: loop source %q: %w", source.NodeID, err)
		}
		sourceResult = &result
	}
	result := loopStepResult(sourceResult, reason, retriesRemaining, attempts)
	output := ""
	if source != nil {
		output = source.Output
	}
	if err := d.appendLoopSettled(u.activation, output, reason, retriesRemaining, result); err != nil {
		return err
	}
	d.record(u.nodeID, output, scope, nodeOutputs)
	return nil
...
// appendSettled emits an outcome.settled event for an activation with no retry
// classification. appendOutcomeSettled derives its typed result from the legacy
// outcome/output/detail fields.
...
// classification used by exec bodies. retryable=false omits that field.
...
// arm stamps (§3.3).
func (d *driver) appendLoopSettled(
	activation, output, reason string,
	retriesRemaining *int,
	result LumenStepResult,
) error {
	return d.appendOutcomeSettled(outcomeSettledPayload{
...
func (d *driver) appendOutcomeSettled(payload outcomeSettledPayload) error {
	payload.Outcome = d.outcomeForDialect(payload.Outcome)
	if payload.Result == nil {
		result := settledPayloadResult(payload)
		payload.Result = &result
	} else {
		payload.Result.Outcome = d.outcomeForDialect(payload.Result.Outcome)
	}
	return d.append(
		EventOutcomeSettled,
		d.streamID+":"+payload.Activation+":settled",
		payload,
	)
}
...
func (d *driver) append(eventType, idemToken string, payload any) error {
	...
	if err := d.store.ApplyDelta(d.ctx, delta); err != nil {
