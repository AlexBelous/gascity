...
## test-conn-oracle: run the opt-in managed Dolt connection-cure process-level oracle (AC-3)
## Proves 200 concurrent generated-default `gc hook --claim` processes routed through the
## controller against a max_connections=32 managed Dolt server bound server connections and
## claim atomically, with the fast path OFF as the accumulating control. Requires dolt + bd.
test-conn-oracle:
	$(TEST_ENV) go test -tags 'integration conn_oracle' -timeout 30m -run 'TestManagedDoltConnOracle' -count=1 ./test/integration
