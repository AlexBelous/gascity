...
func Resume(ctx context.Context, store *graphstore.Store, doc *ir.IR, streamID string, input map[string]any, opts Options) (RunResult, error) {
	...
	if ls.Closed {
		...
		if err != nil {
			return RunResult{}, fmt.Errorf("lumen: resume %q: completed result: %w", streamID, err)
		}
		return RunResult{
			StreamID:    streamID,
			Outcome:     ls.Outcome,
	...
	if err != nil {
		return RunResult{}, fmt.Errorf("lumen: resume %q: formula result: %w", streamID, err)
	}
...
// the dialect but predates skip-dependency classification; v6 predates typed
// step results. All are verified before this function and re-serialized only
// in memory as v7. Every other version is returned unchanged for fold.Fold's
// normal skew rejection.
func snapshotForCurrentReducer(snap fold.Snapshot) (fold.Snapshot, error) {
	if snap.Engine != Engine ||
		(snap.ReducerVersion != 4 &&
			snap.ReducerVersion != 5 &&
			snap.ReducerVersion != 6) {
		return snap, nil
	}
	...
	case 5, 6:
		...
		if err != nil {
			return fold.Snapshot{}, fmt.Errorf("v%d state: %w", snap.ReducerVersion, err)
		}
		state.SemanticDialect = dialect
	}
	for _, node := range state.Nodes {
		if !node.Settled || node.Result != nil {
			continue
		}
		if result, source := resultFromNode(node); source.available() {
			node.Result = &result
		}
	}
	blob, err := state.MarshalSnapshot()
	if err != nil {
		return fold.Snapshot{}, fmt.Errorf("encode v7 state: %w", err)
	}
...
func (d *driver) reapplyDeltas(deltas []fold.Delta) error {
	if err := d.store.ApplyDeltas(d.ctx, deltas); err != nil {
		return fmt.Errorf("lumen: resume: reproject: %w", err)
	}
	return nil
}
