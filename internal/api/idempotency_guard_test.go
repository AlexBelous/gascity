...
var exemptFromIdempotency = map[string]bool{
	...
	// Claim is a conditional mutation (CAS) on an existing bead, not a resource
	// create; upstream ClaimIssue is already idempotent for the same actor, so it
	// needs no Idempotency-Key machinery.
