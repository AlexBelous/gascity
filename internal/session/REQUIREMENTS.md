...
| SESSION-WORK-005 | Late claim cancels queued asynchronous stop | Immediately before an asynchronous provider stop, a reconciler-owned no-wake or orphan drain rechecks every work store reachable by that session. Newly claimed in-progress or ready open work restores the still-running stop-pending session to active and clears the reconciler drain. An unanswerable work query or failed durable cancellation fails closed and does not stop the runtime. Explicit agent-authored drain acknowledgements retain their stop semantics. | `cmd/gc/session_reconciler.go`; `cmd/gc/session_reconciler_test.go` (`TestQueueDrainAckAsyncStopCancelsForLateRigClaim`, `TestQueueDrainAckAsyncStopFailsClosedWhenLateClaimQueryFails`, `TestQueueDrainAckAsyncStopFailsClosedWhenLateClaimCancellationWriteFails`, `TestQueueDrainAckAsyncStopPreservesAgentAuthoredStopSemantics`); `internal/session/store_test.go` (`TestCancelDrainAckStopPendingEmitsSingleUpdateAndReturnsActiveInfo`); `internal/session/lifecycle_transition_test.go` |
