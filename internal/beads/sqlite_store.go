...
func OpenSQLiteStore(dir string, opts ...SQLiteStoreOption) (Store, error) {
	...
	// apply to EVERY pooled connection. The deploy lineage used the
	// mattn-style `?_busy_timeout=` param, which modernc silently ignores
...
func (s *SQLiteStore) depAddTx(ctx context.Context, tx *sql.Tx, issueID, dependsOnID, depType string) error {
	...
	// (beadstest DepAddUpdatesType). The deploy-lineage schema keyed the PK
	// on the type too, which let contradictory duplicate edges accumulate.
	_, err := tx.ExecContext(ctx, `
