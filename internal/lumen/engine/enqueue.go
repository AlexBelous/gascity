...
const (
	// DriverController lets the city controller advance a run and dispatch its
	// ready work to configured agent pools.
	DriverController = "controller"
	// DriverSelf lets one agent session advance and settle the run turn by turn.
	DriverSelf = "self"
)

func normalizeDriver(driver string) (string, error) {
	switch strings.TrimSpace(driver) {
	case "", DriverController:
		return DriverController, nil
	case DriverSelf:
		return DriverSelf, nil
	default:
		return "", fmt.Errorf("lumen: unknown run driver %q (want %s or %s)", driver, DriverController, DriverSelf)
	}
}
...
func EnqueueRun(ctx context.Context, store *graphstore.Store, doc *ir.IR, input map[string]any, formulaRef, defaultRoute string) (string, error) {
	return EnqueueRunWithDriver(ctx, store, doc, input, formulaRef, defaultRoute, DriverController)
}

// EnqueueRunWithDriver is EnqueueRun plus its execution owner. An empty driver is
// accepted as the legacy spelling of DriverController; new events always store the
// explicit value. The discriminator is payload-only, so changing the owner does not
// change the folded formula result.
func EnqueueRunWithDriver(ctx context.Context, store *graphstore.Store, doc *ir.IR, input map[string]any, formulaRef, defaultRoute, driverKind string) (string, error) {
	...
	driverKind, err := normalizeDriver(driverKind)
	if err != nil {
...
type RunManifest struct {
	...
	// Driver is the run's execution owner.
	Driver    string
	CreatedAt string
}
...
func ReadRunManifest(ctx context.Context, store *graphstore.Store, streamID string) (RunManifest, error) {
	...
	driver, err := normalizeDriver(p.Driver)
	if err != nil {
		return RunManifest{}, fmt.Errorf("lumen: run manifest %q: %w", streamID, err)
	}
	return RunManifest{
