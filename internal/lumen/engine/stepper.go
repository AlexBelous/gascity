...
func Step(ctx context.Context, store *graphstore.Store, doc *ir.IR, streamID string, input map[string]any, opts Options) (StepResult, error) {
	...
	if d.st().Closed {
		...
		if err != nil {
			return StepResult{}, fmt.Errorf("lumen: step: completed result: %w", err)
		}
		return StepResult{Done: true, Outcome: d.st().Outcome, Result: &result}, nil
	}
...
func Settle(ctx context.Context, store *graphstore.Store, doc *ir.IR, streamID string, input map[string]any, node, outcome, output string, opts Options) (StepResult, error) {
	...
	if d.st().Closed {
		result, err := d.completedResult(scope)
		if err != nil {
			return StepResult{}, fmt.Errorf("lumen: settle: completed result: %w", err)
		}
		return StepResult{Done: true, Outcome: d.st().Outcome, Result: &result}, nil
	}
...
func (d *driver) stepSeal(units []planUnit, scope map[string]string) (StepResult, error) {
	...
	if err != nil {
		return StepResult{}, fmt.Errorf("lumen: step: formula result: %w", err)
	}
	...
	if err := d.append(EventRunClosed, d.streamID+":run:closed", runClosedPayload{
		Outcome: runOutcome,
	...
	return StepResult{Done: true, Outcome: runOutcome, Result: &result}, nil
}
