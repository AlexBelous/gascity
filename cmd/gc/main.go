...
func openStoreResultAtForCityWithAuthority(storePath, cityPath string, modeOverride gate.Mode, haveMode, authoritative bool) (beads.StoreOpenResult, error) {
	return openStoreResultAtForCityFull(storePath, cityPath, modeOverride, haveMode, authoritative, wrapStoreWithBeadPolicies)
}
...
func openStoreResultAtForCityWith(storePath, cityPath string, wrap func(beads.Store, *config.City) beads.Store) (beads.StoreOpenResult, error) {
	return openStoreResultAtForCityFull(storePath, cityPath, gate.ModeUnset, false, false, wrap)
}

// openStoreResultAtForCityFull carries both the conditional-writes mode/authority
// (from the controller reopen path) and the injectable policy wrapper (work vs
// infra store). The thin wrappers above pin whichever axes they don't vary.
func openStoreResultAtForCityFull(storePath, cityPath string, modeOverride gate.Mode, haveMode, authoritative bool, wrap func(beads.Store, *config.City) beads.Store) (beads.StoreOpenResult, error) {
