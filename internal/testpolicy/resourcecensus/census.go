...
var bootstrapPolicy = Ledger{
	...
	Medium: []MediumOwner{
		...
		},
		{
			...
			PackageName:     "session",
			Owner:           "TestCreateSessionAllowsWorkDirWhenLiveProcessIsUnattributedSquatter",
			Resources:       []Resource{ResourceSubprocess, ResourceFixedSleep},
			OwnerBead:       "ga-esrrot",
			Invariant:       "cwd-collision session-owned-PID scoping regression (ga-thkwp5 Failure 2) is a checked Medium owner",
			ResourceOwner:   "the one real sleep(1) subprocess and its time.Sleep poll are confined to TestCreateSessionAllowsWorkDirWhenLiveProcessIsUnattributedSquatter, which proves liveness scoping against genuine /proc data instead of a fake scanner",
			MigrationTarget: "ga-thkwp5",
			Expires:         "2026-10-01",
		},
	},
