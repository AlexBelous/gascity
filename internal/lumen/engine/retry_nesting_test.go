...
// members and the scatter harvests their outcomes: lane r1 passes on attempt 0;
// lane r2 retryable-fails to its budget (2 attempts) and exhausts. The
// structural scatter still succeeds, retaining both member results as data.
func TestRetryInScatterDrivesBothLanes(t *testing.T) {
	...
	assertSettled(t, settledIDs(t, res.Events), "lanes", engine.OutcomeSucceeded)
}
