...
func loadStatusSessionSnapshot(cityPath string, cfg *config.City, store beads.Store, stderr io.Writer) *sessionBeadSnapshot {
	...
	//
	// On a split city, store is the relocated session (infra) store the
	// cliSessionStore seam above routed to. scopedStoreLike is class-preserving:
	// when the backing dir is the .gc/infra scope root it returns (nil, nil) so
	// we keep reading through the routed store here rather than rebuilding a
	// mis-scoped clone — see scopedStoreLike's infra-scope branch and
	// TestScopedStoreLikeKeepsRelocatedInfraStore.
