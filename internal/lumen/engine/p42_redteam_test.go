...
func TestH1_ScatterAfterGateSkips(t *testing.T) {
	...
	if res.Outcome != engine.OutcomeFailed {
		t.Errorf("run outcome = %q, want failed (the skipped scatter is benign and does not replace A)", res.Outcome)
	}
...
func TestH1_GatherAfterGateSkips(t *testing.T) {
	...
	if res.Outcome != engine.OutcomeSucceeded {
		t.Errorf("run outcome = %q, want succeeded (reviews is the last non-skipped formula value)", res.Outcome)
	}
	if err := store.Verify(ctx, res.StreamID); err != nil {
		t.Errorf("Verify = %v", err)
	}
...
// aggregate settles as a successful harvest, it does not skip.
func TestH1_ScatterMemberFailStillDrains(t *testing.T) {
	...
	if settled["S"] != engine.OutcomeSucceeded {
		t.Errorf("scatter S settled %q, want succeeded harvest", settled["S"])
	}
	if err := store.Verify(ctx, res.StreamID); err != nil {
		t.Errorf("Verify = %v", err)
	}
...
// TestScatterAllMembersSkippedReturnsHarvest pins the upstream structural
// scatter contract: the fan itself succeeds and exposes each skipped member as
// data, even when no member ran.
func TestScatterAllMembersSkippedReturnsHarvest(t *testing.T) {
	ctx := context.Background()
	store := newStore(t)
	...
	if nodeStatus(t, store, "S") != "done" {
		t.Errorf("S status = %q, want done", nodeStatus(t, store, "S"))
	}
	requireResultJSON(t, res.Result,
		`{"value":{"m1":{"value":null,"outcome":"skipped","error":null},"m2":{"value":null,"outcome":"skipped","error":null}},"outcome":"succeeded","error":null}`)
	if err := store.Verify(ctx, res.StreamID); err != nil {
		t.Errorf("Verify = %v", err)
	}
}
...
// --- Structural scatter failures are harvested as data ----------------------

// TestScatterOnFailStopHarvestsFailure proves that structural scatter remains a
// successful harvest even when one member fails under on_fail "stop".
func TestScatterOnFailStopHarvestsFailure(t *testing.T) {
	ctx := context.Background()
	store := newStore(t)
	...
	if settled["bad"] != engine.OutcomeFailed {
		t.Errorf("member bad settled %q, want failed", settled["bad"])
	}
	if settled["S"] != engine.OutcomeSucceeded {
		t.Errorf("scatter S settled %q, want succeeded harvest", settled["S"])
	}
...
// TestScatterAllMembersFailedReturnsSuccessfulHarvest proves that even a total
// member failure is data in a structural scatter's successful result.
func TestScatterAllMembersFailedReturnsSuccessfulHarvest(t *testing.T) {
	ctx := context.Background()
	store := newStore(t)
	...
	settled := settledOutcomeByID(t, res.Events)
	if settled["a"] != engine.OutcomeFailed || settled["b"] != engine.OutcomeFailed {
		t.Errorf("members = %v, want both failed", settled)
	}
	if settled["S"] != engine.OutcomeSucceeded {
		t.Errorf("scatter S settled %q, want succeeded harvest", settled["S"])
	}
	if res.Outcome != engine.OutcomeSucceeded {
		t.Errorf("run outcome = %q, want succeeded", res.Outcome)
	}
	if err := store.Verify(ctx, res.StreamID); err != nil {
		t.Errorf("Verify = %v", err)
	}
...
func TestM3_DanglingAfterRefused(t *testing.T) {
	...
	var journalRows int
	if err := store.ReadDB().QueryRowContext(ctx, `SELECT COUNT(*) FROM journal`).Scan(&journalRows); err != nil {
