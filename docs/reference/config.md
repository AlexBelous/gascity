...
| `work_query_federated` | boolean |  | `false` | Declares that a custom `work_query` already reads every store in the city's routed-work topology. On a city with relocated coordination classes, `gc hook` runs the query once instead of once per bd workspace. An undeclared custom query fails closed on that topology rather than reporting an ambiguous empty queue. |
