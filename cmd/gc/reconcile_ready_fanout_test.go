...
func TestCollectAssignedWorkBeadsCachedMatchesUncached(t *testing.T) {
	...
	wantBeads, _, _, wantReady, wantPartial := collectAssignedWorkBeadsWithStores(&config.City{}, uncachedStore, nil, nil, nil, uncachedSnap)

	cachedStore := &readyQueryRecordingStore{MemStore: beads.NewMemStore()}
	cachedSnap := seed(cachedStore)
	cache := newReadyDemandCache()
	gotBeads, _, _, gotReady, gotPartial := collectAssignedWorkBeadsWithStores(&config.City{}, cachedStore, nil, nil, nil, cachedSnap, cache)
