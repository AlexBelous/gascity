...
//
// This is a SEPARATE config from playwright.config.ts: that one is driven by
// the Go test TestStructuredTranscriptBrowser against an existing dashport
// listener (DASHPORT_BASE_URL), while this one owns its server via webServer.
// testMatch keeps this config scoped to the render smoke so the structured
// transcript spec (which needs the Go-test-seeded session env) never runs here.
//
// The Go binary is prebuilt with -tags integration by the Makefile
// (dashboard-e2e-smoke) or by test:e2e:build; webServer just launches it on a
...
export default defineConfig({
  ...
  webServer: {
    ...
    // run looks wrong, kill the process on PORT (or run `make dashboard-e2e-smoke`
