...
#       (the primary checkout's <repo_root>/.git/gate-slots, shared by linked
#       worktrees; NFR4 fallback, never /tmp — see AGENTS.md Build Cache
#       Conventions).
...
push_gate_slots_dir() {
    ...
    # Resolve through Git rather than appending to <worktree>/.git: in a
    # linked worktree .git is a FILE, so that path cannot contain slots.
    # --git-common-dir also makes every worktree of one repository share the
    # same cap, which is the resource-boundary intent.
    local _pgs_git_common
    _pgs_git_common="$(git rev-parse --git-common-dir 2>/dev/null)" || return 1
    _pgs_git_common="$(cd "$_pgs_git_common" 2>/dev/null && pwd -P)" || return 1
    printf '%s/gate-slots\n' "$_pgs_git_common"
}
