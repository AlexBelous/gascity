...
// graph projection (nodes / edges / frontier) — reducer v7 (blueprint §2). It
...
// Reducer returns the Lumen fold reducer (v7). It is the reducer a store uses to
...
// ReducerVersion reports the stamped reducer version (v7).
...
func applyOutcomeSettled(next *lumenState, e fold.Event) (fold.State, fold.Delta, error) {
	...
	if p.Result != nil {
		result := *p.Result
		n.Result = &result
	} else if result, source := resultFromNode(n); source.available() {
		n.Result = &result
	}
	n.InFrontier = false
