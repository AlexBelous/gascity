...
// TestForEachInRunBodyPerAttemptRefanInline proves a failed post-fan decision
// causes the next run attempt to mint a fresh fan over the same array. Member
// failure remains harvest data; the authored gate controls whether to retry.
func TestForEachInRunBodyPerAttemptRefanInline(t *testing.T) {
	...
	sub := subDoc("reviewer", arrField("arr"),
		forEachNode(nil, "item", "stop", refOver("arr"),
			doNode("mem", "review {{ item }}", nil)),
		doNode("gate", "accept harvest", []string{"fan"}))
	doc := decodeIR(t, bundleDoc(
		arrField("items"),
		repeatRunLoop(nil,
			runNodeRawEnv("stage", nil, "reviewer", `[`+envField("arr", "items")+`]`),
	...
	host := &enginehost.StubHost{Results: map[string]enginehost.DoResult{
		...
		"stage/0/gate":  {Outcome: enginehost.OutcomeFailed, Output: "retry"},
		"stage/1/fan/0": {Outcome: enginehost.OutcomePass, Output: "ok1a"},
		"stage/1/fan/1": {Outcome: enginehost.OutcomePass, Output: "ok1b"},
		"stage/1/gate":  {Outcome: enginehost.OutcomePass, Output: "accepted"},
	}}
	...
	settled := settledOutcomeByID(t, res.Events)
	// Attempt 0 preserves its failed member in a successful fan; the authored gate
	// fails the attempt and causes the loop to retry.
	...
	if settled["stage/0/fan"] != engine.OutcomePass {
		t.Errorf("attempt-0 fan = %q, want pass (member failure is harvest data)", settled["stage/0/fan"])
	}
	if settled["stage/0/gate"] != engine.OutcomeFailed {
		t.Errorf("attempt-0 gate = %q, want failed (authored retry decision)", settled["stage/0/gate"])
	}
	...
	if _, _, _, out := loopSettle(t, res.Events, "loop:0"); out != "accepted" {
		t.Errorf("loop settle output = %q, want accepted (passing attempt result)", out)
	}
	assertProjectionEqualsRefold(t, store, res.StreamID)
}
