...
// TestTimeoutRetryablePreservedAtWrapper proves that a transparent timeout
// returns its body's complete result, including retryability, on both drivers.
func TestTimeoutRetryablePreservedAtWrapper(t *testing.T) {
	ctx := context.Background()
	doc := decodeIR(t, blockDoc("root",
	...
	assertRetryablePreserved(t, inRes.Events, "inline")
	...
	assertRetryablePreserved(t, r.Run.Events, "pool")
}

func assertRetryablePreserved(t *testing.T, events []graphstore.StoredEvent, driver string) {
	t.Helper()
	...
	if present, val := tnkSettleRetryable(t, events, "check:0"); !present || !val {
		t.Errorf("[%s] wrapper check:0 retryable = (present=%v,val=%v), want (true,true)", driver, present, val)
	}
}
...
func TestTimeoutFoldIgnoresDuration(t *testing.T) {
	...
	if v := r.ReducerVersion(); v != 7 {
		t.Fatalf("ReducerVersion() = %d, want 7 (TNK adds no fold state beyond typed results)", v)
	}
}
