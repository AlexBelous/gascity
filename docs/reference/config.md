...
| `terminal_create_cooldown` | string |  | `5m` | TerminalCreateCooldown is the minimum time to wait after a terminal (non-retryable) ephemeral session-create failure for this agent before attempting another fresh create at the same resolved worker directory. Duration string (e.g., "5m", "30m", "1h"). Defaults to "5m". Set to "0s" to disable the throttle. |
