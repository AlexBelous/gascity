...
| SESSION-RECON-015 | Exact-key start ownership rollout | The boot-latched keyed start controller is disabled by default. When selected, it may own only durable pending-create and explicit-wake causes for dependency-free configured sessions; the legacy start wave excludes exactly those owned rows. Admissions are bounded, coalescing hints: every reconcile rereads the exact durable session key, performs no fleet scan, and leaves closed, terminal, blocked, suspended-config, dependency-bearing, or already-running sessions unchanged. Config/provider/store generations are fenced across reload, shutdown drains admission and workers before provider teardown, and unavailable `auto` mode falls back loudly while `require` fails closed. | `cmd/gc/session_start_controller_test.go`; `cmd/gc/session_start_reconcile_test.go`; `cmd/gc/session_start_ownership_test.go`; `cmd/gc/city_runtime_session_start_test.go`; `cmd/gc/session_start_real_bd_tmux_integration_test.go` |
