...
type planUnit struct {
	...
	members      []string // aggregate dependency activation keys
	resultNodes  []ir.Node
	resultPrefix string
...
// binding name, and the single body fanned once per element. The member COUNT is a
// runtime property of the evaluated array (unknown at lowering), so
...
// context (nil/zero for a leaf body). The aggregate returns their exact results as an
// indexed successful harvest. The decision (the array) is frozen by the over-ref gate
// (a member-`over` reads the immutable input LAYER and contributes no gate; a bare-ref
...
// lowerScatter lowers a scatter node.
...
func (l *lowerer) lowerGather(n ir.Node, parent string) error {
	...
	combine, resultNodes, err := l.lowerCombine(n)
	if err != nil {
		return err
	}
	l.units = append(l.units, planUnit{
...
func decodeRunEnv(n ir.Node, inputFields []ir.Field) ([]runEnvField, []string, error) {
	...
	for _, fld := range inputFields {
		if fld.Required && !bound[fld.Name] && !fieldHasDefault(fld) {
...
func (l *lowerer) lowerCombine(n ir.Node) ([]planUnit, []ir.Node, error) {
	raw, ok := n.Raw["combine"]
	if !ok {
		return nil, nil, fmt.Errorf("lumen: gather %q missing combine", n.ID)
	}
	var combine struct {
		Kind  string          `json:"kind"`
	...
	if err := json.Unmarshal(raw, &combine); err != nil {
		return nil, nil, fmt.Errorf("lumen: gather %q combine: %w", n.ID, err)
	}
	if combine.Kind != "authored" {
		// P4.2-deferred: builtin/reduce combine forms.
		return nil, nil, fmt.Errorf("%w: gather combine kind %q (node %q)", ErrUnsupportedNode, combine.Kind, n.ID)
	}
	var block ir.Node
	if err := json.Unmarshal(combine.Block, &block); err != nil {
		return nil, nil, fmt.Errorf("lumen: gather %q combine block: %w", n.ID, err)
	}
	members, err := childNodes(block.Raw["members"])
	if err != nil {
		return nil, nil, fmt.Errorf("lumen: gather %q combine members: %w", n.ID, err)
	}
	...
	for i := range sub.units {
		if sub.units[i].kind != unitLeaf {
			return nil, nil, fmt.Errorf("%w: gather %q combine member %q (kind %q) is not executable in a combine block",
	...
	units, err := topoSortUnits(sub.units)
	return units, members, err
}
...
func mintRunBody(stash runBodyStash, run *runSpec, aggNodeID, prefix, aggActivation, parentActivation, ns string, afterDeps, rawAfter []string, memberIndex *int) ([]planUnit, planUnit, error) {
	...
	agg := planUnit{
		...
		members:      members,
		resultNodes:  stash.bodyFormula.Nodes,
		resultPrefix: prefix,
		memberDeps:   members,
		run:          run,
	}
...
// evalValue renders an IR value expression to a string at a legacy string edge.
// Typed evaluation lives in evaluateLumenExpr; templates and legacy output are
// the only places that coerce its result.
func evalValue(raw json.RawMessage, scope map[string]string) (string, error) {
	value, err := evaluateLumenExpr(raw, stringResultContext(scope))
	if err != nil {
		return "", err
	}
	if value == nil && containsMemberExpr(raw) {
		return "", fmt.Errorf("lumen: unsupported value expression kind %q", "member")
	}
	return promptString(value)
}

func containsMemberExpr(raw json.RawMessage) bool {
	...
	return (expression.Kind == "expr" || expression.Kind == "interp") &&
		containsMemberExpr(expression.Expr)
}

// evalInterp renders an interp node at a legacy string edge.
func evalInterp(raw map[string]json.RawMessage, scope map[string]string) (string, error) {
	return evalInterpWithContext(raw, stringResultContext(scope), scope)
}

func evalInterpWithContext(raw map[string]json.RawMessage, context resultContext, scope map[string]string) (string, error) {
	if v, ok := raw["value"]; ok {
		value, err := evaluateLumenExpr(v, context)
		if err != nil {
			return "", err
		}
		return promptString(value)
	...
	if partsRaw != nil {
		...
		for _, p := range parts {
			...
			value, err := evaluateLumenExpr(expr, context)
			if err != nil {
				return "", err
			}
			rendered, err := promptString(value)
			if err != nil {
				return "", err
			}
