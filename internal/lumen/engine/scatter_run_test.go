...
// TestScatterRunMemberFailureIsHarvested pins the structural scatter contract
// over a run member: the transparent run remains failed, while the enclosing
// scatter succeeds and carries that failure in its harvest.
func TestScatterRunMemberFailureIsHarvested(t *testing.T) {
	ctx := context.Background()
	store := newStore(t)
	doc := decodeIR(t, bundleDoc(
		"",
	...
	if settled["lanes"] != engine.OutcomeSucceeded {
		t.Errorf("scatter lanes settled %q, want succeeded harvest", settled["lanes"])
	}
