...
6. **Two cancel arms are re-derived per key from the probe the FLEET reason was
   ...
   was deliberately NOT ported here on the reasoning that it has no per-key
   analogue and those rows would stay legacy's for the WD wave.

   **SUPERSEDED — the divergence was live, and it is now CLOSED
   (ga-f7v2ft.179).** Those rows did not stay legacy's: under
   `session_reconciler=auto` `advanceSessionDrainsExcluding` skips any row the
   keyed controller holds a drain-advance admission for, so nothing applied the
   cancel and the drain ran to `state.deadline` and force-stopped a session the
   fleet scan would have spared — for the seven `WakeReason` values the two
   narrow arms do not cover (config, create, session, keep-warm, attached, wait,
   pin). The premise was also wrong in its own terms: a fleet verdict does not
   need a per-key ANALOGUE, it needs the fleet's ANSWER, which is the shape
   `DesiredSessionNames`, `ProviderHealth` and `SessionLiveness` already take.
   The sweep now publishes its `awakeSetToWakeEvals` projection and the handler
   reads its own key out of it; an absent view or an absent key declines, which
   is the fleet scan's own answer for a row missing from `wakeEvals`.
   RED: `TestExactDrainAdvanceCancelsForAnyReacquiredWakeReason`, with
   `TestExactDrainAdvanceKeepsDrainsTheFleetScanAlsoRefusesToCancel` and
   `TestExactDrainAdvanceDeclinesTheWakeCancelWithoutAPublishedView` as its
   controls. **§3b's D-DRAIN row is updated to match.**

   **CORRECTION (ga-f7v2ft.161, council finding B2). The .179 closure covered
   six of the seven reasons, not all seven: `attached` was still unreachable,
   and the CLOSED marker above overstated what shipped.** The published
   projection is built by `detectorAwakeSet`, which sets
   `AwakeInput.AttachedSessions` to an empty map on purpose — attachment is a
   per-session provider probe the sweep may not pay fleet-wide (§2; §3b D-SLEEP
   "probe/pending arms unpredicted"). `ComputeAwakeSet` therefore never reaches
   its `"attached"` rung and `awakeSetToWakeEvals` never emits `WakeAttached`, so
   the view-reading arm could not rescue an attached session no matter what the
   tick observed. The .179 tests passed because each hand-seeded a
   `SessionWakeEvaluations` view; none drove a real publication. The live effect
   was the one .179 named: under `session_reconciler=auto` the fleet scan skips
   the row, so a keyed idle drain force-stopped a session a person was attached
   to. The same vacancy carried `PendingSessions`, but D-DRAIN's cancel arm 1
   already re-pays `pendingInteractionKeepsAwakeInfo` handler-side, so `pending`
   was never view-only.

   **Fixed HANDLER-SIDE, not by widening the sweep**, per the rule the rest of
   §3 follows: the fleet refuses fleet-wide provider I/O, the handler buys the
   one probe for the one key it holds. Cancel arm 3 now takes its verdict from
   the published view OR from `exactSessionUserAttached` — the attachment rung
   `exactSessionActiveUseDeferralReason` already uses for D-ORPHAN's (WD.4) and
   D-SLEEP's (WD.5) drain arms, extracted so the three families cannot drift on
   what counts as an attached human. The view is read first so the free map read
   decides before a provider read is bought, the gate stays
   `drainReasonCancelable` (an attached execution-stalled seat is still not
   rescued, exactly as in the fleet scan), and the trace carries
   `wake_cancel_source` so `gc trace` distinguishes the two sources.
   RED: `TestExactDrainAdvanceRescuesAnAttachedSessionThroughTheRealPublication`
   (driven by a real `detectorAwakeSet` publication, with the unattached leg as
   the control that must reach stop-pending),
   `TestDetectorWakeProjectionCannotCarryAttachment` (the structural proof, with
   the legacy bridge's `WakeAttached` on the same row and the same provider as
   its control), and
   `TestExactDrainAdvanceAttachmentRescueRespectsTheFleetScansCancelGate`.
   The two detector omissions are now NAMED exemptions in
   `awakeInputFieldExemptions` (`cmd/gc/twin_drift_census_test.go`), each citing
   its handler-side re-payment site per family; the census that certified the
   vacancy counted an empty composite literal as population and no longer does.
...
| D-STALE-CREATE | decision | rollback vs preserved | legacy defers rollback #6+ (R6 budget retired); **`pending_create_in_flight_family_split`** — a start-pending row with a live create lease is claimed by the sweep's preserve arm (session_detector_sweep.go:1285-1289, `predicted_effect: none`) while the start it already began is driven in the D-WAKE family, so one row lands in two families at two sites and no same-site join can pair them (WD.15, cross-checked on the twin's presence in the cycle). The twin has TWO spellings, because the start's driver depends on who holds the key: legacy drives it itself at :4101 (reason `wake`), or — under keyed start ownership — stands down (`keyed_start_owner`, the row-scan skip at :1880-1887 and the wake-target stand-down at :4091-4098) and the keyed start controller drives it. Either record proves legacy scanned the row this cycle AND that the start family is handling it. No D-STALE-CREATE yield can stand in: the family's own stand-down (`keyed_stale_create_owner`, :1835-1846) is gated on `pendingCreateLeaseExpiredForRollbackInfo`, i.e. the ROLLBACK arm only, so a live lease — the preserve arm's whole premise — can never produce one (WD.15 day-2); **`live_runtime_recovery_excluded_from_sweep`** — legacy defers a pending-create recovery only for a row whose runtime is ALIVE (:3117-3125) and `detectStaleCreate` excludes exactly those rows (:1272-1274), so the legacy-only record is structural |
...
| D-DRAIN | detection | tracker-state candidacy (drain intent / draining) | **`drain_ack_adjacent_cycle_convergence`** (OWNER-SIGNED 2026-08-17/18, WD.15 day 7) — the ack-timing skew (handler-side ack read vs legacy's in-tick poll), scoped to legacy's OWN acknowledgement arm and incomparable ONLY ON PROOF. Each skew must individually prove its keyed twin: same session, same `reconciler.session.drain_ack` site, an ADJACENT cycle, within one tick. The window is 11.5s — one tick at the slower of the campaign's two measured cadences (median inter-tick gap 9.222s over 9010 consecutive pairs on 2026-08-15, 11.454s over 8178 on 2026-08-17) — because the claim the class makes is that the ack and the poll are two observations of the SAME drain episode, and the reconciler re-decides a session's drain every tick. The keyed side is identified by the OWNERSHIP STAMP (`effect_owner=keyed`), never by `detector_family`: the keyed handler's ack operations carry no family label, so a `detector_family` predicate matches nothing. A TWINLESS skew stays an UNCLASSIFIED MISMATCH and blocks WE — the class cannot blanket the site, because a legacy ack with no keyed ack beside it is legacy writing an acknowledgement the keyed engine never wrote, which is the divergence D-DRAIN exists to catch. Adjudicated per row rather than by rule because the shape is structural and far too rare to characterise statistically: ~2/day against ~90 comparable joins/day. Signed specimens: `dependent-rc-7mzpx` cycle-0860a236ff1b82bd (2026-08-15T04:49:21.053Z), `s-rc-wisp-y73064d` cycle-41b467cd627d719e (2026-08-17T00:21:48.259Z), `s-rc-wisp-d30uo8f` cycle-03d51f88678dbb50 (2026-08-17T02:05:29.315Z); **advance arms journey-proven**, which covers two shapes WD.15 separated: the sweep's due-advance detection (`detector_drain_in_flight`, sited at DrainAck) has no legacy per-session twin at all because legacy's advance pass is the `session_reconcile.drain_advance` PHASE site (§1 row 28), and legacy's own engine arms (drain.stale/cancel/timeout, session_wake.go:686-825) sit one site away from that detection; ~~**keyed declines the plain wake-reasons-reappeared cancel legacy applies** (WD.6 delta 6)~~ — **CLOSED (ga-f7v2ft.179)**. The premise was that the fleet `len(eval.Reasons) > 0` verdict has no per-key analogue. It now has one: the sweep publishes its `awakeSetToWakeEvals` projection and the keyed advance reads the same fleet answer for its own key, so the keyed arm applies the cancel legacy applies. The class was never merely a reporting gap — under `session_reconciler=auto` the fleet scan SKIPS a row the keyed controller holds an advance admission for, so those rows were not "staying legacy's", they were reaching their drain deadline and being force-stopped. Rows previously triaged into this class are matches from a0815625bb on, and any campaign window that predates it must be re-read rather than carried forward. **CORRECTED (ga-f7v2ft.161, council B2): a0815625bb closed SIX of the seven reasons, not all seven.** `attached` stayed structurally unreachable through the published view — `detectorAwakeSet` leaves `AwakeInput.AttachedSessions` empty by design, so `ComputeAwakeSet` never reaches its `"attached"` rung and the projection cannot carry `WakeAttached` — and an attached session went on being force-stopped under `auto` until the handler-side re-pay landed (cancel arm 3 now also consults `exactSessionUserAttached`, the rung D-ORPHAN and D-SLEEP already use). Rows in the `attached` shape are matches only from THAT commit on; the other six are matches from a0815625bb as stated; keyed refuses on incomplete liveness where legacy completes the drain (WD.6 delta 7) |
...
| D-STALL | decision | claim-less stall + floor exemption | claim-check-error fail-safe arm incomparable; keyed refuses a pinned configured named row where legacy sets-then-clears the marker (WD.12 delta 2); the named circuit-breaker clear now travels with the recycle, so WD.12 delta 9's divergence is CLOSED (WD.11 delta 9); **`reset_stall_alarm_no_detector_arm`** — `ResetStalled` is in this family's site list by adjacency, not membership. `recordResetStallIfDue` fires only for a NOT-alive row whose committed reset outlived the startup timeout (session_reconciler.go:223) while `detectStall` returns unless `live.Alive` (session_detector_sweep.go:1710), so the populations are disjoint by construction; and the site is an ALARM — stderr + `SessionResetStalled` event + trace, zero mutation — so there is no effect for a keyed handler to own, which makes it incomparable rather than a counted mismatch. Note this family is also off by default (`progress_stall_timeout` unset ⇒ gate 0 ⇒ that early return), i.e. the §6/Q3 test-only-parity case (WD.15 day-2) |
