...
func TestEmitCompletedFromClosedNotificationUsesPhysicalSnapshot(t *testing.T) {
	...
	step := mustCreateLifecycleStep(t, graph, "gcg-retry-attempt", root.ID, "build", `["prepare"]`)
	step.Status = "closed"
...
func TestReconcileCompletedRepairsMissingFactAndRetainsConflictingHistory(t *testing.T) {
	graph := beads.NewMemStore()
	root := mustCreateLifecycleRoot(t, graph)
	step := mustCreateLifecycleStep(t, graph, "gcg-attempt", root.ID, "build", `["prepare"]`)
	closed := "closed"
	...
	if got := ReconcileCompleted(recorder, graph, "execution-reconcile"); got != 1 {
		t.Fatalf("ReconcileCompleted = %d, want 1 correction", got)
	}
	if got := ReconcileCompleted(recorder, graph, "execution-reconcile"); got != 0 {
...
func mustCreateLifecycleRoot(t *testing.T, store beads.Store) beads.Bead {
	t.Helper()
...
func mustCreateLifecycleStep(t *testing.T, store beads.Store, id, rootID, stepID, topology string) beads.Bead {
	t.Helper()
	...
	step, err := store.Create(beads.Bead{ID: id, Metadata: metadata})
	if err != nil {
		t.Fatal(err)
	}
	return step
