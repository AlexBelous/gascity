...
func deliverSlingNudge(target nudgeTarget, sp runtime.Provider, store beads.Store, cityPath string, stdout, stderr io.Writer) {
	...
	if err := enqueueQueuedNudgeWithStore(target.cityPath, beads.NudgesStore{Store: resolveNudgesStore(store, target.cfg, target.cityPath, nil)}, newQueuedNudgeWithOptions(target.agent.QualifiedName(), msg, "sling", now, queuedNudgeOptionsFromTarget(target))); err != nil {
