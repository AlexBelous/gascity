...
The indirect-listener catalog recognizes direct `syscall.Socket`/`Bind` setup,
`net.FileListener`/`FilePacketConn` descriptor duplication, the exact
`acp.(*Provider).Start` and `subprocess.(*Provider).Start` methods, and the
conditional `cliauth.(*Client).Login` and `supervisor.LoadConfig` paths.
`Login` and `LoadConfig` are conservative conditional proxies: a counted
invocation reaches code that may bind a listener, but is not proof that the
invocation actually opened one.
Method identity requires the canonical production type and pointer-method
declarations. Same-package constructor results are resolved from a bounded
catalog of sibling declarations; imported constructors are seeded only when
the exact production declaration is present. The scanner retains types only
for cataloged receiver expressions and never loads repository packages or
toolchain export data. The source census is 66 calls in 6 files overall and 65
calls in 5 untagged files. Dolt and other shared-host resources remain explicit
follow-up catalogs. A Medium resource
...
direct `syscall.Socket` and `syscall.Bind`; descriptor-backed
`net.FileListener` and `net.FilePacketConn`; exact
`acp.(*Provider).Start`, `subprocess.(*Provider).Start`, and
`cliauth.(*Client).Login` methods; `supervisor.LoadConfig`;
...
`net/http/httptest`, `internal/cliauth`, `internal/runtime/acp`,
`internal/runtime/runtimecapability`, `internal/runtime/subprocess`,
`internal/runtime/tmux`, `internal/supervisor`,
`test/acceptance/helpers`, or `test/tmuxtest` are
...
| --- | --- | --- | --- | --- | --- | --- |
| Audit baseline | all tracked test source | fixed_sleep: 440 calls / 158 files (historical regex census: 447 / 157) | ga-80po0c.2 | tracked test source totals remain visible as audit evidence; ga-80po0c.2 owns this point-in-time source census | P0.4a | 2026-10-01 |
| Audit baseline | all tracked test source | listener_indirect: 66 calls / 6 files | ga-80po0c.2.2.4 | tracked indirect-listener totals remain visible as audit evidence; ga-80po0c.2.2.4 owns this point-in-time indirect-listener source census | P0.4c-listener-indirect | 2026-10-01 |
...
| Small debt ratchet | all untagged test source | listener_indirect: 65 calls / 5 files | ga-80po0c.2.2.4 | untagged Small indirect-listener call/file totals cannot grow; reductions must lower this baseline; non-Medium lexical owners replace descriptor, provider, or conditional listeners or declare exact isolated ownership | P0.4c-listener-indirect | 2026-10-01 |
...
| Source debt ratchet | all untagged test source | listener_indirect: 65 calls / 5 files | ga-80po0c.2.2.4 | untagged indirect-listener call/file totals cannot grow; reductions must lower this baseline; each owning test replaces descriptor, provider, or conditional listeners or moves the retained boundary to exact Medium ownership | P0.4c-listener-indirect | 2026-10-01 |
