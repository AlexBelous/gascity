...
func TestAdvanceScatterPoolMembersFanOutAndAggregate(t *testing.T) {
	...
	t.Run("one_member_fails_harvest_succeeds", func(t *testing.T) {
		ctx := context.Background()
		store := newStore(t)
		...
		// One member fails; the other two pass. The structural scatter still
		// succeeds and preserves the member failure in its result harvest.
		...
		if o := settledOutcomeOf(t, store, streamID, "lanes:0"); o != engine.OutcomeSucceeded {
			t.Fatalf("aggregate lanes:0 settled %q, want succeeded harvest", o)
		}
		if r2.Run.Outcome != engine.OutcomeSucceeded {
			t.Fatalf("run outcome = %q, want succeeded", r2.Run.Outcome)
		}
		if err := store.Verify(ctx, streamID); err != nil {
			t.Fatalf("Verify: %v", err)
		}
