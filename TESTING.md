...
| --- | --- | --- | --- | --- | --- | --- |
| Audit baseline | all tracked test source | fixed_sleep: 428 calls / 157 files (historical regex census: 447 / 157) | ga-80po0c.2 | tracked test source totals remain visible as audit evidence; ga-80po0c.2 owns this point-in-time source census | P0.4a | 2026-10-01 |
...
| Audit baseline | all tracked test source | subprocess: 542 calls / 169 files (historical regex census: 495 / 135) | ga-80po0c.2 | tracked test source totals remain visible as audit evidence; ga-80po0c.2 owns this point-in-time source census | P0.4a | 2026-10-01 |
...
| Source debt ratchet | all untagged test source | subprocess: 396 calls / 112 files (historical regex census: 380 / 98) | ga-80po0c.2 | untagged subprocess call/file totals cannot grow; reductions must lower this baseline; each process-owning test removes or replaces its source call site | D1/D2/D5/D6/E6 | 2026-10-01 |
...
By default, the local runners bound concurrency by detected CPUs, available
memory, and the remaining hierarchical cgroup task budget. They budget 4 GiB
and 1024 tasks per heavy job, then cap automatic fan-out at 16. The task budget
is detected after the runner enrolls in its test slice and walks from that
runner cgroup to its controller root, so an unlimited transient scope still
honors a finite ancestor such as a systemd slice's `TasksMax`.
If either finite resource budget cannot be detected safely, they use at most
three jobs. An explicitly supplied `LOCAL_TEST_JOBS` always wins:
...
under `<city_root>/.gc/gate-slots` (or `<git_common_dir>/gate-slots` outside
a city, shared across linked worktrees) before running any jobs, and holds it for the invocation's entire
