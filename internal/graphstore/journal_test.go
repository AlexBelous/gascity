...
// TestChainHashTamperEvidence proves the hash chain binds every payload. The
// test drops the write-closure trigger to model out-of-band file tampering.
func TestChainHashTamperEvidence(t *testing.T) {
	...
	if _, err := db.ExecContext(ctx,
		`UPDATE journal SET payload = ? WHERE stream_id = ? AND seq = 2`,
		divergent, stream,
	); err != nil {
		t.Fatalf("tamper with seq 2: %v", err)
	}
...
// TestAppendOnlyTriggerRejectsUpdateDelete pins the permanent write closure:
// neither UPDATE nor DELETE has a production bypass.
func TestAppendOnlyTriggerRejectsUpdateDelete(t *testing.T) {
	...
	if err == nil || !containsAppendOnly(err.Error()) {
		t.Fatalf("DELETE journal = %v, want append-only rejection", err)
	}
}
