...
// the pinned gh and Dolt sources still select vulnerable x/text versions. The residual
...
func TestTrivyIgnoreDropsStdlibWaiversForRebuiltTools(t *testing.T) {
	...
	allowedRebuiltToolWaivers := map[string]map[string]bool{
		"CVE-2026-56852": {
			"usr/bin/gh":         true,
			"usr/local/bin/dolt": true,
		},
	...
	for _, v := range doc.Vulnerabilities {
		for _, p := range v.Paths {
			...
			if allowedPaths, ok := allowedRebuiltToolWaivers[v.ID]; ok && allowedPaths[p] {
	...
	for cve, paths := range allowedRebuiltToolWaivers {
