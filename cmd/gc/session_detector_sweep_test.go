...
// D-WAKE's NAMED and CONFIGURED-DEPENDENCY arms at WD.10a, with its pool-fill
// arm still shadow-only until WD.10b. A family that flips an act constant
// without an arm in detectorAdmissionSourceFor — or with an arm but no landed
// handler — fails here before it can double-act beside a non-yielding legacy.
