...
installing the pinned Playwright browser, or run all layers with `make
...
is NOT in the Go integration shard set. Run it with `make dashboard-e2e-smoke`
...
`make dashboard-e2e` runs all layers. In CI it runs as appended steps in the
