...
func (s *Store) Append(ctx context.Context, streamID, engine string, expectedVersion, leaseEpoch uint64, events []JournalEvent) (AppendResult, error) {
	...
	err := s.write(ctx, func(tx *sql.Tx) error {
		...
		// A pure replay is acknowledged even when expectedVersion is stale.
		if len(fresh) == 0 {
			result.FirstSeq = result.Duplicates[0]
			return nil
		}

		head, previous, err := headAndChain(ctx, tx, streamID)
		if err != nil {
			return err
		}
		if head != expectedVersion {
			return fmt.Errorf(
				"graphstore: append: stream %q head %d, expected %d: %w",
				streamID, head, expectedVersion, ErrWrongExpectedVersion,
			)
		}
		...
		if head == 0 {
			previous = genesisHash(streamID, s.cityID)
		}

		appendedAt := time.Now().UTC().Format(time.RFC3339Nano)
		seq := head
		for _, index := range fresh {
			event := events[index]
			seq++
			payloadHash := canon.Hash(event.Payload)
			chain := chainHash(
				previous,
				streamID,
				seq,
			...
			if _, err := tx.ExecContext(ctx,
				`INSERT INTO journal
				 ...
				 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
				streamID, seq, event.Substream, engine, event.Type, event.IRContractVersion,
				nullableToken(event.IdemToken), event.Payload, payloadHash[:], chain[:],
				leaseEpoch, appendedAt,
			); err != nil {
				return fmt.Errorf("graphstore: append: insert seq %d: %w", seq, err)
			}
			if index == fresh[0] {
				result.FirstSeq = seq
			}
...
// Verify recomputes every surviving payload and chain hash. A stream that
// starts above seq 1 must have a snapshot cut anchor for the removed prefix.
func (s *Store) Verify(ctx context.Context, streamID string) error {
	...
	if events[0].Seq == 1 {
		...
		var ok bool
		prev, ok, err = s.cutChainHashAt(ctx, streamID, events[0].Seq-1)
		if err != nil {
			return err
		}
		if !ok {
			return fmt.Errorf(
				"graphstore: verify %q: stream starts at seq %d without cut anchor: %w",
				streamID,
				events[0].Seq,
				ErrChainBroken,
			)
		}
		wantSeq = events[0].Seq
	}
