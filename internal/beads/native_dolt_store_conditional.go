...
func (s *NativeDoltStore) CloseWithMetadataIfMatch(id string, expectedRevision int64, metadata map[string]string) (Bead, error) {
	...
	for attempt := 1; attempt <= nativeWriteAttempts; attempt++ {
		var closed Bead
		...
		if !isNativeDoltSerializationConflict(err) || attempt == nativeWriteAttempts {
			return Bead{}, nativeStoreError(id, err)
		}
		time.Sleep(time.Duration(attempt) * nativeWriteRetryBackoff)
	}
	return Bead{}, nativeStoreError(id, err)
}
