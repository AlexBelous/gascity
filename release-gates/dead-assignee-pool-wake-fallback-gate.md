...
| 3 | Tests pass | PASS (non-regression exception) | `gofmt -l` on all 8 changed files: clean. `go vet ./...`: clean. `go build ./...`: clean. `git diff --check $(git merge-base origin/main HEAD) HEAD`: clean. Focused acceptance sweep `go test ./cmd/gc/... -run 'TestFilterAssignedWorkBeadsForPoolDemand|TestBuildDesiredState|TestDeadAssignee' -count=1 -v`: PASS (`ok github.com/gastownhall/gascity/cmd/gc 2.364s`). `make test-fast-parallel` ran all fast shards: `unit-core`, `fsys-darwin-compile`, and `unit-cmd-gc` shards 2-6 passed; shard 1 failed only `TestErrorReturningSessionProviderFactoriesPreserveSuccessBehavior/default`. The same test fails on current `origin/main` (`89c96220a`) with the same assertion, and is tracked as `ga-y4se3w`; it is unrelated to this `cmd/gc` pool-demand diff. |
...
`make test-fast-parallel` is locally red because the environment reproduces
...
This is classified as a pre-existing mainline/nested-live-city test failure,
not a regression from the deploy diff.
