...
func doBd(args []string, stdout, stderr io.Writer) int {
	...
	// gc-only "topology" subcommand: inspect/set the work-bead topology. Handled
	// entirely here (read-only show + dry-run + atomic city.toml set) BEFORE the
	// bd passthrough and its fail-closed marker guard — an operator must be able
	// to INSPECT a city whose config already contradicts its markers.
	if len(bdArgs) > 0 && bdArgs[0] == "topology" {
		return doBdTopology(cityName, bdArgs[1:], stdout, stderr)
	}
