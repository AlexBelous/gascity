...
// settleDrainAckLateAssignedWork leaves a short commit/replication window
// between the durable stop-pending transition and the final assigned-work
// fence. A claim can already be in flight when stop-pending is emitted; without
// this window, the final read can still win by a few milliseconds and kill the
// worker immediately before that claim becomes visible.
func settleDrainAckLateAssignedWork() {
	time.Sleep(2 * time.Second)
}

// cancelDrainAckAsyncStopForLateAssignedWork closes the gap between the
// reconciler's no-work decision and the detached provider kill. Only
// reconciler-owned no-wake/orphan drains are cancelable here; an explicit
// agent-authored drain ack keeps its stop semantics.
//
// A true result means the caller must skip the kill. Query and persistence
// errors deliberately return true: uncertainty at this final destructive
// boundary fails closed and the next level-triggered controller tick retries.
func cancelDrainAckAsyncStopForLateAssignedWork(
	cityPath string,
	store beads.Store,
	...
	reason, reconcilerOwned := reconcilerDrainAckMatchesSessionInfo(info, sp, name)
	if !reconcilerOwned || !assignedWorkDrainReasonCancelable(reason) {
		return false
	}

	hasAssignedWork, err := sessionHasAwakeAssignedWorkForReachableStore(cityPath, cfg, store, rigStores, info)
	if err != nil {
		fmt.Fprintf(stderr, "session reconciler: async drain-ack stop %s skipped: late assigned-work check failed: %v\n", name, err) //nolint:errcheck
		return true
	}
	if !hasAssignedWork {
		return false
	...
	// Fence 2 from engdocs/design/session-store-fences.md: the detached action
	// re-reads the current session row and refuses to mutate a replacement
	// incarnation. The provider token fence above protects the runtime target;
	// this bead-side fence protects the durable cancellation write.
	freshInfo, err := sessionFrontDoor(store).Get(info.ID)
	if err != nil {
		fmt.Fprintf(stderr, "session reconciler: async drain-ack stop %s skipped: refreshing late-claim session: %v\n", name, err) //nolint:errcheck
		return true
	}
	if !isDrainAckStopPendingInfo(freshInfo) {
		fmt.Fprintf(stderr, "session reconciler: async drain-ack stop %s skipped: late claim found after stop-pending state changed\n", name) //nolint:errcheck
		return true
	}
	if expectedToken != "" && freshInfo.InstanceToken != "" && freshInfo.InstanceToken != expectedToken {
		fmt.Fprintf(stderr, "session reconciler: async drain-ack stop %s skipped: late-claim session token mismatch (session was replaced)\n", name) //nolint:errcheck
		return true
	}
	if _, err := sessionFrontDoor(store).CancelDrainAckStopPending(freshInfo); err != nil {
		fmt.Fprintf(stderr, "session reconciler: async drain-ack stop %s skipped: persisting late-claim drain cancellation: %v\n", name, err) //nolint:errcheck
		return true
	}
	if err := clearReconcilerDrainAckMetadata(sp, name); err != nil {
		fmt.Fprintf(stderr, "session reconciler: async drain-ack stop %s: clearing late-claim drain metadata: %v\n", name, err) //nolint:errcheck
	}
	fmt.Fprintf(stderr, "session reconciler: async drain-ack stop %s canceled: assigned work appeared before kill\n", name) //nolint:errcheck
	return true
}

func queueDrainAckAsyncStop(cityPath string, store beads.Store, rigStores map[string]beads.Store, sp runtime.Provider, cfg *config.City, info sessionpkg.Info, name, expectedToken string, processNames []string, lateClaimSettle func(), tracker *asyncStartTracker, stderr io.Writer) {
	...
	key := drainAckAsyncStopKey(info.ID, name)
	...
	go func() {
		...
		// A reconciler-owned no-wake/orphan stop is cancelable when work lands
		// late. Give an already-running claim mutation enough time to become
		// visible before the final work query. Agent-authored acknowledgements do
		// not enter this window and retain their prompt stop semantics.
		if reason, reconcilerOwned := reconcilerDrainAckMatchesSessionInfo(info, sp, name); reconcilerOwned && assignedWorkDrainReasonCancelable(reason) && lateClaimSettle != nil {
			lateClaimSettle()
			// The runtime name can be reused while the goroutine settles. Re-check
			// the token immediately afterward so the final query/kill never targets
			// a replacement incarnation.
			if expectedToken != "" {
				if actualToken, _ := sp.GetMeta(name, "GC_INSTANCE_TOKEN"); actualToken != "" && actualToken != expectedToken {
					fmt.Fprintf(stderr, "session reconciler: async drain-ack stop %s skipped after late-claim settle: instance token mismatch (session was replaced)\n", name) //nolint:errcheck
					return
				}
			}
		}
		if cancelDrainAckAsyncStopForLateAssignedWork(cityPath, store, rigStores, sp, cfg, info, name, expectedToken, stderr) {
			_ = poke(cityPath)
			return
		}
...
func reconcileDrainAckStopPending(
	...
	if err != nil || obs.Running || obs.Alive {
		// Async-stop mutates only through its detached, token-fenced final check;
		// this reconcile snapshot therefore stays coherent and a zero result
		// (applyTo no-op) matches the caller-visible row. The full typed Info is
		// carried into that check so a late claim can durably cancel a
		// reconciler-owned no-wake/orphan stop before the provider kill.
		queueDrainAckAsyncStop(cityPath, store, rigStores, sp, cfg, info, name, info.InstanceToken, tp.Hints.ProcessNames, settleDrainAckLateAssignedWork, asyncStopTracker, stderr)
...
func finalizeDrainAckStopPendingSessions(
	...
	for _, info := range infos {
		...
		if err != nil {
			...
			queueDrainAckAsyncStop(cityPath, store, rigStores, sp, cfg, info, name, info.InstanceToken, processNames, settleDrainAckLateAssignedWork, asyncStopTracker, stderr)
			continue
		}
	...
	for i := range orderedRows {
					...
					if acked, _ := dops.isDrainAcked(name); acked {
						...
						if providerAlive {
							...
							if updated, ok := markDrainAckStopPending(infoByID[id], sessFront, clk, stderr); ok {
								...
								queueDrainAckAsyncStop(cityPath, store, rigStores, sp, cfg, infoByID[id], name, infoByID[id].InstanceToken, tp.Hints.ProcessNames, settleDrainAckLateAssignedWork, asyncStopTracker, stderr)
								if trace != nil {
		...
		if gateThreshold := minPositiveDuration(claimlessThreshold, claimHolderThreshold); gateThreshold > 0 && alive && sessionActivityReportable(sp, name) {
			...
			if lastActivityErr == nil && !lastActivity.IsZero() && clk.Now().Sub(lastActivity) > gateThreshold {
				...
				// because they parked on an error. That exemption applies only to
				// the claim-less recycler: a city that opts into claim-holder
				// recovery must still look up a floor worker's claim before it can
				// decide whether it is really idle.
				floorExempt := false
				if !exempt && cfg != nil {
