...
func TestFoldRunViewProjectsSealedRun(t *testing.T) {
	...
	if view.Result == nil {
		t.Fatal("Result = nil, want the folded typed formula result")
	}
	if view.Result.Outcome != engine.OutcomeSucceeded || view.Result.Value != "refined" {
		t.Fatalf("Result = %#v, want succeeded value refined", view.Result)
	}
