...
// TestEmitDueComputeFactsSweepsLiveKeylessWispInInfraStore is THE decisive
// composition this deploy-line base makes testable: on the real fleet the token
// producers are KEYLESS (no session_key) ephemeral wisp codex sessions in the INFRA
// store. A live keyless wisp must be reached via the split-city all-tier (TierBoth)
// infra enumeration AND have its rollout resolved by the ambiguity-guarded
// (cwd, wake-window) workdir fallback — while still awake — so its model calls are
// counted in near-real-time. The earlier fork base had no keyless fallback, so this
// path could not exist there.
func TestEmitDueComputeFactsSweepsLiveKeylessWispInInfraStore(t *testing.T) {
	cityPath := t.TempDir()
	workDir := t.TempDir()
	codexRoot := t.TempDir()
	sinkPath := filepath.Join(cityPath, ".gc", "usage.jsonl")
	...
	start := time.Date(2026, 6, 15, 10, 0, 0, 0, time.UTC)
	// A keyless rollout in this wisp's unique worktree — only cwd + wake window resolve
	// it (no session_key keys it).
	writeKeylessCodexRolloutForSweep(t, codexRoot, start, workDir, "019e7777-cccc-7000-8000-00000000000f", [][3]int{
		{150, 100, 50},
		{450, 200, 100},
	})
	...
	b, err := infra.Create(beads.Bead{
		...
		Title:     "live keyless codex wisp session",
		Labels:    []string{session.LabelSession},
		Metadata: map[string]string{
			"state":            "active", // AWAKE, still producing
			"session_name":     "codex-wisp-1",
			"awake_started_at": start.Format(time.RFC3339),
			"work_dir":         workDir,
			...
			"builtin_ancestor": "codex",
			"molecule_id":      "run-W",
			// NB: NO session_key and NO slept_at — a LIVE keyless wisp, the real fleet shape.
		},
	})
	if err != nil {
		t.Fatal(err)
	...
	// Empty snapshot: the ephemeral keyless wisp is reachable only via the all-tier
	// infra enumeration, and its rollout only via the keyless workdir fallback.
	...
	if got := kindCount(facts, usage.KindModel); got != 2 {
		t.Fatalf("model facts = %d, want 2 (a LIVE KEYLESS wisp in the infra store must be swept via the workdir fallback); facts: %+v", got, facts)
	}
	if got := kindCount(facts, usage.KindCompute); got != 0 {
		t.Fatalf("compute facts = %d, want 0 (still awake)", got)
	}
	for _, f := range facts {
		if f.Kind == usage.KindModel && f.Provider != "codex" {
			t.Fatalf("model fact Provider = %q, want codex (wrapped name resolved via builtin_ancestor)", f.Provider)
...
// TestEmitDueComputeFactsSweepsLiveWispSessionInInfraStore is the KEYED wisp variant
// of the producer path: an ephemeral, session-keyed live wisp in the INFRA store,
// reached only via the split-city all-tier enumeration (empty main snapshot), is
// swept incrementally while awake.
func TestEmitDueComputeFactsSweepsLiveWispSessionInInfraStore(t *testing.T) {
	...
	if got := kindCount(facts, usage.KindModel); got != 2 {
		t.Fatalf("model facts = %d, want 2 (live wisp swept via all-tier infra enumeration); facts: %+v", got, facts)
	}
	if got := kindCount(facts, usage.KindCompute); got != 0 {
		t.Fatalf("compute facts = %d, want 0 (still awake)", got)
	}
	...
	if got := refreshed.Metadata[usageModelSweptAtKey]; got != "" {
		t.Fatalf("usage_model_swept_at = %q, want empty (live sweep leaves the interval open)", got)
	}
	if _, err := workStore.Get(b.ID); err == nil {
		t.Fatal("work store must not contain the wisp session bead (it lives in the infra store)")
	}
...
// complete records count and the cursor advances only through them; the torn trailing
// line is not counted and the cursor does not advance onto it — so once the writer
// finishes the line, the next tick counts it exactly once.
...
// regression the two arms share a cursor for, on the SPLIT-CITY producer path: a wisp
// is swept live for a while, then retires and the terminal arm sweeps its final
...
func TestEmitDueComputeFactsLiveThenRetirementNoDoubleCountWisp(t *testing.T) {
	...
	// Tick 1 (awake): the live arm sweeps the two invocations so far (all-tier infra
	// enumeration; empty snapshot).
	cr.emitDueComputeFacts(context.Background(), nil)
	facts1, _, err := usage.ReadFacts(sinkPath)
	if err != nil {
		t.Fatalf("ReadFacts (tick 1): %v", err)
...
// discovery: with the memo pre-seeded to a specific rollout, the sweep reads THAT path
// and never runs discovery — which would otherwise resolve the session's own keyed
// rollout (with different tokens). Distinct token values make the choice observable.
func TestLiveModelSweepUsesMemoizedPathSkippingDiscovery(t *testing.T) {
	...
	// discoverable one would have TWO (100/50, 200/100). Seeing exactly the seeded one
	// proves discovery was skipped.
	if got := kindCount(facts, usage.KindModel); got != 1 {
