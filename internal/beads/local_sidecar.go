...
// to a JSON file on disk, keyed by bead ID then key. It backs on-disk stores
// whose clone-local values must remain outside their durable database. The
// sidecar never participates in Dolt sync, topology migration, or the bd
// subprocess path.
//
...
// CloneLocalStrings is the narrow persistence surface for clone-local Store
// values. It is exported for on-disk Store implementations in sibling
// internal packages; values remain outside their durable database.
type CloneLocalStrings interface {
...
// NewCloneLocalStrings returns a lazily loaded clone-local sidecar. An empty
// path produces an in-memory-only implementation.
func NewCloneLocalStrings(path string) CloneLocalStrings {
	return newLocalSidecar(path)
}
...
// Clear removes every clone-local value.
func (l *localSidecar) Clear() error {
	l.mu.Lock()
	defer l.mu.Unlock()
	if err := l.ensureLoadedLocked(); err != nil {
		return err
