...
// TestForEachInSubFormulaAggregateShadowsBinding proves a settled scatter result
// shadows a same-named environment binding. The harvest is a record, so the
// downstream for-each fails invalid_input instead of reading the input array.
func TestForEachInSubFormulaAggregateShadowsBinding(t *testing.T) {
	ctx := context.Background()
	store := newStore(t)
	...
	if settled["stage/fan"] != engine.OutcomeFailed {
		t.Fatalf("sub-fan stage/fan = %q, want failed (the harvest is not an array)", settled["stage/fan"])
	}
	if got := res.NodeOutputs["stage/fan/0"]; got != "" {
		t.Errorf("member 0 = %q, want empty (the harvest shadows the binding)", got)
	}
}
...
// TestForEachInSubFormulaOnFailStopStillReturnsHarvest proves a failed member
// remains data when the fan runs inside a namespace.
func TestForEachInSubFormulaOnFailStopStillReturnsHarvest(t *testing.T) {
	ctx := context.Background()
	store := newStore(t)
	...
	settled := settledOutcomeByID(t, res.Events)
	if settled["stage/fan"] != engine.OutcomePass {
		t.Errorf("aggregate stage/fan = %q, want pass (failed member is harvest data)", settled["stage/fan"])
	}
}

// TestForEachInSubFormulaOnFailContinueReturnsHarvest proves the enclosing run
// transparently returns the successful fan harvest.
func TestForEachInSubFormulaOnFailContinueReturnsHarvest(t *testing.T) {
	ctx := context.Background()
	store := newStore(t)
	fail := `if [ "{{ item }}" = "bad" ]; then exit 1; fi; echo ok`
	sub := subDoc("reviewer", arrField("arr"),
	...
	if settled["stage"] != engine.OutcomePass {
		t.Errorf("run stage = %q, want pass (transparent from the fan harvest)", settled["stage"])
	}
}

// TestForEachInSubFormulaAggregateVisibility proves the harvest, successful
// outcome, and dependency settlement are all visible inside a namespace.
func TestForEachInSubFormulaAggregateVisibility(t *testing.T) {
	...
	wantHarvest := `v={"0":{"value":"x","outcome":"succeeded","error":null}}`
	if got := res.NodeOutputs["stage/probe"]; got != wantHarvest {
		t.Errorf("template-form render probe = %q, want harvest %q", got, wantHarvest)
	}
}
