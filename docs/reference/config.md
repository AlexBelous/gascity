...
| `session_reconciler` | string |  | `off` | SessionReconciler selects keyed exact-start ownership during the rollout. "off" (the default) keeps all session work on the legacy fleet-wide reconciler. "auto" selects exact-start when available and otherwise degrades loudly. Status healing remains legacy-owned in every mode. "require" refuses startup rather than falling back when those requirements are unavailable. The value is boot-latched; changing it requires a controller restart. Enum: `off`, `auto`, `require` |
